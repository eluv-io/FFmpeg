...
ifeq ($(WINDOWS), FALSE)
	ifeq ($(UNAME), Darwin)
		INCLUDES= -pthread ${OPTFLAG} -lm
	        ...
	        INCLUDES= -pthread -lrt ${OPTFLAG} -lm -ldl
	endif
else
	INCLUDES= ${OPTFLAG} -lws2_32 -Wl,--stack,4194304
endif
...
all:${ALL_OBJECTS}
	...
	${CC} -shared -o $(OBJS_PATH)/${TARGET_LIB_SHARED}.${TARGET_VERSION} $(LINK_OBJECTS) ${INCLUDES}
	${CC} -o $(OBJS_PATH)/${TARGET} $(OBJS_PATH)/ni_device_test.o $(LINK_OBJECTS) ${INCLUDES}
ifeq ($(WINDOWS), FALSE)
	${CC} -o $(OBJS_PATH)/${TARGETP2P} $(OBJS_PATH)/ni_p2p_test.o $(LINK_OBJECTS) ${INCLUDES}
	${CC} -o $(OBJS_PATH)/ni_libxcoder_dynamic_loading_test $(OBJS_PATH)/ni_libxcoder_dynamic_loading_test.o $(LINK_OBJECTS) ${INCLUDES}
endif
	${CC} -o $(OBJS_PATH)/test_rsrc_api $(OBJS_PATH)/test_rsrc_api.o $(LINK_OBJECTS) ${INCLUDES}
	${CC} -o $(OBJS_PATH)/ni_rsrc_namespace $(OBJS_PATH)/ni_rsrc_namespace.o $(LINK_OBJECTS) ${INCLUDES}
...
# dependence
%.o : ${SRC_PATH}/%.cpp
	${CC} ${CXXFLAGS} ${INCLUDES} -c $< -o ${OBJS_PATH}/$@
