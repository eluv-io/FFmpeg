 ...
 *******************************************************************************/
void print_perf(ni_device_type_t module_type,
                ni_device_queue_t *coders,
                ni_session_context_t *sessionCtxt)
{
  ...
  if (module_type == NI_DEVICE_TYPE_DECODER)
  {
    ...
    if(!dev_ctxt_arr)
    {
      fprintf(stderr, "ERROR %d: Failed to allocate memory for ni_rsrc_mon print_perf()",
              NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR);
      return;
    }
...
int main(int argc, char *argv[])
{
  ...
  while (!stop_process)
  {
    ...
    ni_usleep((int64_t)checkInterval * 1000 * 1000);
  }
