...
{                                                                    \
  ...
  if (err_rc_info.sess_err_no || ctx->rc_error_count >= NI_XCODER_FAILURES_MAX || \
      (tmp_rc = ni_nvme_check_error_code(rc, opcode, type, hw_id, inst_id))) { \
...
{                                                                    \
  ...
  int tmp_rc = NI_RETCODE_FAILURE; \
  if (info.sess_err_no || ctx->rc_error_count >= NI_XCODER_FAILURES_MAX || \
      (tmp_rc = ni_nvme_check_error_code(rc, opcode, type, hw_id, inst_id))) { \
    ni_log(NI_LOG_INFO, "Persistent failures detected, %s() line-%d: session_no 0x%x sess_err_no %u " \
           "inst_err_no %u rc_error_count: %d\n", __FUNCTION__, __LINE__, *inst_id, \
...
int32_t worker_q_enq(queue_info *local_queue_info, worker_queue_item worker_q_item)
{
  ...
  else
  {
    ...
    ni_log(NI_LOG_TRACE, "enq opcode %x tailidx %d\n",
           local_queue_info->worker_queue_head[local_queue_info->tailidx].opcode, 
...
void *workerthread_decoder_read(void *p_ctx)
{
  ...
  while(1){
    ...
    if (local_ctx->hw_action == NI_CODEC_HW_DOWNLOAD)
    {
      WRITE_INSTANCE_SET_DW11_INSTANCE(worker_q_item_local->p_ni_nvme_cmd->cdw11, local_ctx->hwd_src_cpu);
      WRITE_INSTANCE_SET_DW12_ISHWDESC(worker_q_item_local->p_ni_nvme_cmd->cdw12,
                                       (local_ctx->hwd_minor_offset << NI_CODEC_HW_PAYLOAD_OFFSET)
                                       | NI_CODEC_HW_ENABLE | NI_CODEC_HW_DOWNLOAD); //xcoder will upload YUV
      WRITE_INSTANCE_SET_DW12_FRAMEINSTID(worker_q_item_local->p_ni_nvme_cmd->cdw12, local_ctx->hwd_Frame_Idx);
      hwd_offset_adjust = local_ctx->hwd_chunk_offset; //offset to Quadra appears larger than offset to p_data on host
      //ni_debug_print( "hwdl_read %d %d\n", local_ctx->hwd_Frame_Idx, local_ctx->hwd_Inst_ID);
    }
    else
    {
      WRITE_INSTANCE_SET_DW12_ISHWDESC(worker_q_item_local->p_ni_nvme_cmd->cdw12, NI_CODEC_HW_NONE);
      WRITE_INSTANCE_SET_DW11_INSTANCE(worker_q_item_local->p_ni_nvme_cmd->cdw11, NI_DEVICE_TYPE_DECODER);
    }

    WRITE_INSTANCE_SET_DW11_PAGEOFFSET(worker_q_item_local->p_ni_nvme_cmd->cdw11,
                                       worker_q_item_local->frame_chunk_index);  // set page index based on 4096 bytes.
    WRITE_INSTANCE_SET_DW15_SIZE(worker_q_item_local->p_ni_nvme_cmd->cdw15, worker_q_item_local->data_len);

    ni_log(NI_LOG_TRACE, "decoder_read opcode %d, handle %d, p_ni_nvme_cmd dw10:0x%8x dw11:0x%8x dw15:0x%8x,"
           "  data_len %d, chunk idx %d, session ID %d\n",
    ...
    while((localret!=0)){
        ...
        localret = ni_nvme_send_io_cmd_thru_admin_queue(nvme_admin_cmd_xcoder_read, worker_q_item_local->handle,
                     worker_q_item_local->p_ni_nvme_cmd, worker_q_item_local->data_len,
                     ((worker_q_item_local->p_data)+(worker_q_item_local->frame_chunk_index - hwd_offset_adjust)*FRAME_CHUNK_INDEX_SIZE),
                     worker_q_item_local->p_result);
#else
        localret = ni_nvme_send_io_cmd(worker_q_item_local->opcode, worker_q_item_local->handle,
                     worker_q_item_local->p_ni_nvme_cmd, worker_q_item_local->data_len,
                     ((worker_q_item_local->p_data)+(worker_q_item_local->frame_chunk_index - hwd_offset_adjust)*FRAME_CHUNK_INDEX_SIZE),
                     worker_q_item_local->p_result);
#endif
        CHECK_ERR_RC(local_ctx, localret, worker_q_item_local->opcode, local_ctx->device_type,
                     local_ctx->hw_id, &(local_ctx->session_id));
...
void *workerthread_encoder_read(void *p_ctx)
{
  ...
  while (1)
  {
    ...
    WRITE_INSTANCE_SET_DW11_PAGEOFFSET(worker_q_item_local->p_ni_nvme_cmd->cdw11,
                                       worker_q_item_local->frame_chunk_index);  // set page index based on 4k
    WRITE_INSTANCE_SET_DW12_ISHWDESC(worker_q_item_local->p_ni_nvme_cmd->cdw12, NI_CODEC_HW_NONE);
    WRITE_INSTANCE_SET_DW15_SIZE(worker_q_item_local->p_ni_nvme_cmd->cdw15, worker_q_item_local->data_len);
...
void *workerthread_encoder_write(void *p_ctx)
{
  ...
  while (1)
  {
    ...
    WRITE_INSTANCE_SET_DW11_PAGEOFFSET(worker_q_item_local->p_ni_nvme_cmd->cdw11,
                                       worker_q_item_local->frame_chunk_index);  // set page index based on 4k
    WRITE_INSTANCE_SET_DW12_ISHWDESC(worker_q_item_local->p_ni_nvme_cmd->cdw12,
                                     (local_ctx->hw_action == NI_CODEC_HW_UPLOAD) ? NI_CODEC_HW_UPLOAD : 0);
...
#ifdef MSVC_BUILD
    ni_log(NI_LOG_TRACE,"Worker thread encoder write opcode %d, handle %d, p_ni_nvme_cmd "
           "dw10:0x%8x dw11:0x%8x dw15:0x%8x, data_len %d, chunk idx %d, session ID %d dw14_yuv_offset %d\n",
           worker_q_item_local->opcode,
           worker_q_item_local->handle,
           worker_q_item_local->p_ni_nvme_cmd->cdw10,
           worker_q_item_local->p_ni_nvme_cmd->cdw11,
...
#else
    ni_log(NI_LOG_TRACE,"TID:%lu Encoder write opcode %d, handle %d, p_ni_nvme_cmd "
           "dw10:0x%8x dw11:0x%8x dw15:0x%8x, data_len %d, chunk idx %d, session ID %d dw14_yuv_offset %d\n",
           pthread_self(),
           worker_q_item_local->opcode,
           worker_q_item_local->handle,
    ...
    ni_log(NI_LOG_TRACE,"encoder_write_processed_data_len in thread %d %d\n",
           local_ctx->encoder_write_processed_data_len,
           local_ctx->encoder_write_total_num_of_bytes);
    //here we check if the last chunk in the frame had been sent out
...
static void decoder_dump_dir_open(ni_session_context_t* p_ctx)
{
  ...
  p_device_context = ni_rsrc_get_device_context(p_ctx->device_type,
                                                p_ctx->hw_id);
  if (! p_device_context)
  {
...
ni_retcode_t ni_decoder_session_open(ni_session_context_t* p_ctx)
{
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ...
    ((ni_get_session_id_t *)p_buffer)->session_id = NI_INVALID_SESSION_ID;

    // Get session ID
    ui32LBA = OPEN_GET_SID_R(0, NI_DEVICE_TYPE_DECODER);
    ...
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): nvme read command failed, blk_io_handle:"
             "%" PRIx64 ", hw_id, %d\n", (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id);
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
    p_ctx->session_id = ni_ntohl(((ni_get_session_id_t *)p_buffer)->session_id);
    ni_log(NI_LOG_TRACE, "Decoder open session ID:0x%x\n",p_ctx->session_id);
    if (NI_INVALID_SESSION_ID == p_ctx->session_id)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): query session ID failed, "
             "p_ctx->blk_io_handle=%" PRIx64 ", p_ctx->hw_id=%d, p_ctx->session_id=%d\n",
    ...
    {
      ni_log(NI_LOG_TRACE, "Decoder open session low_delay_mode %d\n", low_delay_mode);
      low_delay_mode = 1;
    }

    session_info.low_delay_mode = ni_htonl(low_delay_mode);
    session_info.hw_desc_mode = ni_htonl(p_ctx->hw_action);
    memcpy(p_buffer, &session_info, sizeof(ni_decoder_session_open_info_t));
    ui32LBA = OPEN_SESSION_W(0, p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
    retval = ni_nvme_send_write_cmd(p_ctx->blk_io_handle, p_ctx->event_handle,
                                    p_buffer, NI_DATA_BUFFER_LEN, ui32LBA);
    CHECK_ERR_RC(p_ctx, retval, NI_NO_CHECK_TS_NVME_CMD_OP,
    ...
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): nvme write command failed, "
             "blk_io_handle: %" PRIx64 ", hw_id, %d\n",
             (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id);
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
    ...
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): nvme write keep_alive_timeout "
             "command failed, blk_io_handle: %" PRIx64 ", hw_id, %d\n",
             (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id);
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
    ...
    // Later if the model load is exceeding uint32 max,
    // then we can start to divide some value in order to make it fit to uint32
    ...
    if (ni_nvme_send_admin_cmd(nvme_admin_cmd_xcoder_open, p_ctx->device_handle, &cmd, buffer_size,
                               p_signature, &(p_ctx->session_id)))
    {
      ni_log(NI_LOG_TRACE, "ERROR ni_decoder_session_open(): p_ctx->device_handle=%" PRIx64 " , "
    ...
    ni_log(NI_LOG_TRACE, "ni_decoder_session_open(): p_ctx->device_handle=%" PRIx64 ", "
           "p_ctx->hw_id=%d, p_ctx->session_id=%d\n",
           (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
  }
  ...
  if (p_ctx->p_session_config)
  {
    ...
    ni_params_print(p_param, NI_DEVICE_TYPE_DECODER);
  }
  ...
  if (((ni_encoder_params_t*)p_ctx->p_session_config)->nb_save_pkt)
  {
...
ni_retcode_t ni_decoder_session_close(ni_session_context_t* p_ctx, int eos_recieved)
{
  ...
  for (counter = 0; counter< MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    if (p_ctx->ThreadID_decoder_read[counter])
    {
      ...
      if (ret)
      {
        ni_log(NI_LOG_ERROR, "ERROR %d: decoder read join failed:%d\n",
               NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR, ret);
        ni_log(NI_LOG_ERROR, "Count:%d, ThreadID:%d\n",
  ...
  {
    ni_log(NI_LOG_TRACE, "ni_decoder_session_close(): p_ctx->blk_io_handle=%" PRIx64 ", "
    ...
    else
    {
      ...
      ni_usleep(NI_SESSION_CLOSE_RETRY_INTERVAL_US); // 500000 us
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
    }
    retry++;
  }
  ...
  int retry = 0;
  while (retry < NI_MAX_SESSION_CLOSE_RETRIES)
  {
    ni_log(NI_LOG_TRACE, "ni_decoder_session_close(): p_ctx->device_handle=%" PRIx64 ", "
           "p_ctx->hw_id=%d, p_ctx->session_id=%d, close_mode=1\n",
           (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
  ...
  {
    uint32_t ni_usage = (uint32_t)((p_ctx->codec_total_ticks / NI_VPU_FREQ) * 100 /
  ...
  ni_log(NI_LOG_TRACE, "decoder total_pkt:%" PRIu64 ", total_ticks:%" PRIu64 ", total_time:%" PRIu64 " us\n",
         p_ctx->frame_num,
...
int ni_decoder_session_write(ni_session_context_t* p_ctx, ni_packet_t* p_packet)
{
  ...
  while (1)
  {
        ...
        inst_info.wr_buf_avail_size < packet_size)
    {
      ...
      ni_usleep(NI_RETRY_INTERVAL_100US);  // 100 us
    }
    else
    {
...
#ifdef XCODER_IO_RW_ENABLED
  ui32LBA = WRITE_INSTANCE_W(0, p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
#else
  ...
  if (p_packet->p_data)
  {
    ni_log(NI_LOG_TRACE, "ni_decoder_session_write() had data to send: packet_size=%u, "
           "p_packet->sent_size=%d, p_packet->data_len=%d, p_packet->start_of_stream=%d, "
           "p_packet->end_of_stream=%d, p_packet->video_width=%d, p_packet->video_height=%d\n",
           packet_size, p_packet->sent_size, p_packet->data_len, p_packet->start_of_stream,
    ...
    retval = ni_nvme_send_write_cmd(
      ...
      NI_DATA_BUFFER_LEN, CONFIG_INSTANCE_SetPktSize_W(0, p_ctx->session_id,
    ...
    if (((ni_encoder_params_t*)p_ctx->p_session_config)->nb_save_pkt)
    {
      ...
      ((ni_encoder_params_t*)p_ctx->p_session_config)->nb_save_pkt) + 1;
    ...
    while (packet_size > 0)
    {
      ...
      ni_log(NI_LOG_TRACE, "ni_decoder_session_write(): packet_size=%u, write_size_bytes=%u, "
             "p_packet->sent_size=%d, p_packet->start_of_stream=%d, p_packet->end_of_stream=%d, retry=%u\n",
             packet_size, write_size_bytes, p_packet->sent_size, p_packet->start_of_stream,
             p_packet->end_of_stream, send_count);
      ...
      WRITE_INSTANCE_SET_DW12_ISHWDESC(cmd.cdw12, p_ctx->hw_action & NI_CODEC_HW_ENABLE);
...
#ifdef XCODER_OLD_NVME_DRIVER_ENABLED
      retval = ni_nvme_send_io_cmd_thru_admin_queue(nvme_admin_cmd_xcoder_write, p_ctx->blk_io_handle, &cmd,
                                                    write_size_bytes, p_data, &nvme_result);
#else
      retval = ni_nvme_send_io_cmd(nvme_cmd_xcoder_write, p_ctx->blk_io_handle, &cmd, write_size_bytes,
                                   p_data, &nvme_result);
#endif
      CHECK_ERR_RC(p_ctx, retval, nvme_cmd_xcoder_write,
                   p_ctx->device_type, p_ctx->hw_id,
      ...
      if (NI_RETCODE_SUCCESS != retval)
      {
        ...
        if (send_count >= NI_MAX_DEC_SESSION_WRITE_QUERY_RETRIES)
        {
          if (p_ctx->status == NI_RETCODE_NVME_SC_WRITE_BUFFER_FULL)
          {
            ni_log(NI_LOG_TRACE, "WARNING decoder: sending pkt %" PRIu64 " getting in write BUF_FULL "
                   "condition again after previous BUF_FULL; nvme_result=%u, p_packet->sent_size=%d, "
                   "p_packet->data_len=%d, p_ctx->sent_size=%d\n",
          ...
          p_ctx->status = NI_RETCODE_NVME_SC_WRITE_BUFFER_FULL;
          ni_log(NI_LOG_TRACE, "ni_decoder_session_write(): result=%u, status=%d, p_packet->sent_size=%d, "
                 "p_packet->data_len=%d, p_ctx->sent_size=%d\n",
  ...
  if (p_ctx->is_dec_pkt_512_aligned)
  {
    ...
    if (p_ctx->is_first_frame && (p_ctx->pkt_index != -1))
    {
      ...
      {
        ni_log(NI_LOG_DEBUG, "ni_decoder_session_write(): more than NI_MAX_DEC_REJECT frames "
               "are rejected by the decoder. Increase NI_MAX_DEC_REJECT is required or "
               "default gen pts values will be used !\n");
  ...
  else
  {
    ...
    {
      p_ctx->pkt_custom_sei[p_ctx->pkt_index % NI_FIFO_SZ] = malloc(sizeof(ni_all_custom_sei_t));
      if (p_ctx->pkt_custom_sei[p_ctx->pkt_index % NI_FIFO_SZ])
      {
        memcpy(p_ctx->pkt_custom_sei[p_ctx->pkt_index % NI_FIFO_SZ], p_packet->p_all_custom_sei,
      ...
      else
      {
        ...
        ni_log(NI_LOG_ERROR, "%s: failed to allocate custom SEI buffer for pkt, error=%d.\n",
               __func__, NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR);
      }
    }
    else
    {
  ...
  if (NI_RETCODE_SUCCESS == retval)
  {
    ni_log(NI_LOG_TRACE, "ni_decoder_session_write(): exit: packets: %" PRIu64 " offset %" PRIu64 " "
...
int ni_decoder_session_read(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
  ...
  int i = 0;
  int retval = NI_RETCODE_SUCCESS;
  int metadata_hdr_size = NI_FW_META_DATA_SZ - sizeof(ni_hwframe_surface_t);
  int sei_size = 0;
  int frame_cycle = 0;
  ...
  while (1)
  {
    ...
    else if (0 == inst_info.rd_buf_avail_size)
    {
      ...
      if (p_ctx->ready_to_close)
      {
        ...
        retval = ni_query_stream_info(p_ctx, NI_DEVICE_TYPE_DECODER, &data, 0);
      ...
      if (p_ctx->decoder_low_delay && inst_info.frames_dropped && p_ctx->is_first_frame)
      {
  ...
  unsigned int bytes_read_so_far = 0;
    ...
    ui32LBA = READ_INSTANCE_R(0, p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
    ...
    else
    {
      ...
      ni_log(NI_LOG_TRACE, "decoder read success, size %d totall_bytes_to_read include sei %d sei_size %d frame_cycle %d\n", 
             retval, totall_bytes_to_read, sei_size, frame_cycle);
    }
  ...
  ni_log(NI_LOG_TRACE, "total_num_of_chunksdr %d totall_bytes_to_read %d max_nvme_io_size %d ylen %d "
  ...
  {
    actual_read_size = read_size_bytes = (totall_bytes_to_read <= p_ctx->max_nvme_io_size) ?
                                         totall_bytes_to_read : p_ctx->max_nvme_io_size;
    ...
    if (!one_chunk_flag)
    {
      for(;frame_chunk_counter < p_ctx->decoder_read_total_num_of_chunks -1;frame_chunk_counter++)
      {
        ...
        p_ctx->args_decoder_read[frame_chunk_counter].data_len = (frame_remain_bytes < p_ctx->max_nvme_io_size) ?
                                                                 (frame_remain_bytes) : (p_ctx->max_nvme_io_size);
        ...
        p_ctx->args_decoder_read[frame_chunk_counter].frame_chunk_index = ((frame_chunk_counter*p_ctx->max_nvme_io_size) /
        ...
        ni_log(NI_LOG_TRACE, "Decoder read frame_chunk_counter %d totall_bytes_to_read %d "
               "p_ctx->decoder_read_total_num_of_chunks %d chunk index %d frame_remain_bytes %d\n",
               frame_chunk_counter,
               totall_bytes_to_read,
               p_ctx->decoder_read_total_num_of_chunks,
    ...
    WRITE_INSTANCE_SET_DW11_PAGEOFFSET(last_pkt_chunk_read.p_ni_nvme_cmd->cdw11,
                                       last_pkt_chunk_read.frame_chunk_index);  // set page index based on 4k
    WRITE_INSTANCE_SET_DW12_ISHWDESC(last_pkt_chunk_read.p_ni_nvme_cmd->cdw12,
                                     NI_CODEC_HW_NONE);
    WRITE_INSTANCE_SET_DW15_SIZE(last_pkt_chunk_read.p_ni_nvme_cmd->cdw15,
                                 last_pkt_chunk_read.data_len);  // this one we read all the way end to the buffer

    int32_t ret_last = -1;
    int32_t retry_count = 0;
    uint32_t data_len_local = last_pkt_chunk_read.data_len;
    while(ret_last != 0){
      ni_log(NI_LOG_TRACE, "decoder_rread opcode %d, handle %d, p_ni_nvme_cmd dw10:0x%8x dw11:0x%8x "
             "dw15:0x%8x,  data_len %d, pdataoff %p \n",
      ...
      {
        last_pkt_chunk_read.data_len = ( (last_pkt_chunk_read.data_len / NI_MEM_PAGE_ALIGNMENT) * 
      ...
      else
      {
        ...
        *(last_pkt_chunk_read.p_result) =  data_len_local;
        ...
        ni_log(NI_LOG_TRACE, "decoder read success, size %d local %d totall_bytes_to_read "
               "include sei %d sei_size %d frame_cycle %d\n", 
               *(last_pkt_chunk_read.p_result), data_len_local,
               totall_bytes_to_read, sei_size, p_meta->frame_cycle);
        break;
      }
    }
  ...
  if (rx_size > 0)
  {
    ...
    if (ni_timestamp_get_with_threshold(p_ctx->dts_queue, 0, (int64_t*)& p_frame->dts,
                                        XCODER_FRAME_OFFSET_DIFF_THRES, (decq_count % 500 == 0),
    ...
    else
    {
      ...
      else
      {
        ...
        if (frame_offset == 0)
        {
          ...
          {
            p_frame->p_custom_sei = (uint8_t *)p_ctx->pkt_custom_sei[0];
        ...
        else
        {
          for (i = 0 ; i < NI_FIFO_SZ ; i++)
          {
                ...
                &&(frame_offset  < p_ctx->pkt_offsets_index[i]))
            {
              ...
              {
                p_frame->p_custom_sei = (uint8_t *)p_ctx->pkt_custom_sei[i % NI_FIFO_SZ];
  ...
  ni_log(NI_LOG_TRACE, "xcoder_dec_receive(): p_frame->start_of_stream=%d, p_frame->end_of_stream=%d, "
  ...
  if ((p_frame->dts != NI_NOPTS_VALUE) && (p_ctx->frame_time_q != NULL)) {
      ...
      ni_log(NI_LOG_INFO, "DTS:%lld,DELTA:%lu,dLAT:%lu;\n",
             ...
             ni_lat_meas_q_check_latency(p_ctx->frame_time_q, abs_time_ns, p_frame->dts));
...
ni_retcode_t ni_decoder_session_query(ni_session_context_t* p_ctx)
{
  ...
  if (NI_RETCODE_SUCCESS == retval)
  {
    ...
    ni_log(NI_LOG_TRACE, "ni_decoder_session_query current_load:%d fw_model_load:%d "
           "fw_video_mem_usage:%d active_contexts %d\n",
...
ni_retcode_t ni_encoder_session_open(ni_session_context_t* p_ctx)
{
  ...
  // init the queue with encoder write stack, reset all the head and tail
  worker_q_init(&(p_ctx->encoder_write_workerqueue), p_ctx->worker_queue_encoder_write);
  // init the queue with encoder read stack, reset all the head and tail
  worker_q_init(&(p_ctx->encoder_read_workerqueue), p_ctx->worker_queue_encoder_read);
#endif
  ...
  {
    int c;
    ni_device_handle_map_SN(p_ctx->sender_handle, &p_ctx->d_serial_number);
#ifdef XCODER_IO_RW_ENABLED
    ni_device_handle_map_SN(p_ctx->blk_io_handle, &p_ctx->e_serial_number);
#else
    ni_device_handle_map_SN(p_ctx->device_handle, &p_ctx->e_serial_number);
#endif
    for (c = 0; c < 20; c++)
    {
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ...
    // Get session ID
    ((ni_get_session_id_t *)p_buffer)->session_id = NI_INVALID_SESSION_ID;
    ui32LBA = OPEN_GET_SID_R((p_ctx->hw_action == NI_CODEC_HW_ENABLE), NI_DEVICE_TYPE_ENCODER);
    retval = ni_nvme_send_read_cmd(p_ctx->blk_io_handle, p_ctx->event_handle,
                                   p_buffer, NI_DATA_BUFFER_LEN, ui32LBA);

    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_encoder_session_open(): nvme read command failed, blk_io_handle: "
             "%" PRIx64 ", hw_id, %d\n", (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id);
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
    ...
    if (NI_INVALID_SESSION_ID == p_ctx->session_id)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_encoder_session_open(): query session ID failed, "
             "p_ctx->blk_io_handle=%" PRIx64 ", p_ctx->hw_id=%d, p_ctx->session_id=%d\n",
             (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id, p_ctx->session_id);
    ...
    ui32LBA = OPEN_SESSION_W((p_ctx->hw_action == NI_CODEC_HW_ENABLE), p_ctx->session_id, NI_DEVICE_TYPE_ENCODER);
    retval = ni_nvme_send_write_cmd(p_ctx->blk_io_handle, p_ctx->event_handle,
                                    p_buffer, NI_DATA_BUFFER_LEN, ui32LBA);
    CHECK_ERR_RC(p_ctx, retval, NI_NO_CHECK_TS_NVME_CMD_OP,
                 p_ctx->device_type, p_ctx->hw_id,
    ...
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_encoder_session_open(): nvme write command failed, blk_io_handle: "
             "%" PRIx64 ", hw_id, %d\n", (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id);
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
    ...
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_encoder_session_open(): nvme write keep_alive_timeout command failed, "
             "blk_io_handle: %" PRIx64 ", hw_id, %d\n", (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id);
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
    ...
    if (NI_INVALID_SESSION_ID == p_ctx->session_id)
    {
      ni_log(NI_LOG_TRACE, "ERROR: no InstanceID received, ni_encoder_session_open(): p_ctx->device_handle="
  ...
  {
    ni_log(NI_LOG_ERROR, "ERROR: calling ni_config_instance_set_encoder_params(): p_ctx->device_handle="
           "%" PRIx64 ", p_ctx->hw_id=%d, p_ctx->session_id=%d\n",
           (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
    ni_encoder_session_close(p_ctx, 0);
    retval = NI_RETCODE_ERROR_INVALID_SESSION;
  ...
  while (1)
  {
    ...
    else
    {
      ...
      ni_usleep(NI_ENC_SESSION_OPEN_RETRY_INTERVAL_US);  // 1000us
    }
  }
...
ni_retcode_t ni_encoder_session_close(ni_session_context_t* p_ctx, int eos_recieved)
{
  ...
  for (counter = 0; counter< MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    if (p_ctx->ThreadID_encoder_write[counter])
    {
      ...
      if (ret)
      {
        ni_log(NI_LOG_ERROR, "ERROR %d: encoder write thread join failed:%d\n",
               NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR, ret);
        ni_log(NI_LOG_ERROR, "Count:%d, ThreadID:%d\n",
  ...
  ni_log(NI_LOG_TRACE, "encoder total_pkt:%" PRIu64 ", total_ticks:%" PRIu64 ", total_time:%" PRIu64 " us\n",
         p_ctx->pkt_num,
         p_ctx->codec_total_ticks,
         (codec_end_time - p_ctx->codec_start_time));
...
int ni_encoder_session_write(ni_session_context_t* p_ctx, ni_frame_t* p_frame)
{
  ...
  else //handle regular frame sending
  {
    if (p_frame->p_data)
    {
      ...
      uint32_t ui32LBA = WRITE_INSTANCE_W(ishwframe, p_ctx->session_id, NI_DEVICE_TYPE_ENCODER);
      ni_log(NI_LOG_TRACE, "Encoder session write: p_data = %p, p_frame->buffer_size = %u, p_ctx->frame_num = "
     ...
     // while (frame_size_bytes > 0)
      {
        ...
        {
          ni_log(NI_LOG_TRACE, "WARN: tx_size > NI_MEM_PAGE_ALIGNMENT and tx_size != NI_MAX_FRAME_CHUNK_SZ,"
                 " tx_size %% NI_MEM_PAGE_ALIGNMENT = %u\n", tx_size % NI_MEM_PAGE_ALIGNMENT);
          if (tx_size % NI_MEM_PAGE_ALIGNMENT)
          {
        ...
        if (!one_chunk_flag){
          for(; frame_chunk_counter < total_num_of_chunks-1; frame_chunk_counter++)
          {
              ...
              p_ctx->args_encoder_write[frame_chunk_counter].data_len = ((frame_remain_bytes) < (p_ctx->max_nvme_io_size)) ?
                                                                         frame_remain_bytes :((p_ctx->max_nvme_io_size));
              ...
              // address of set that is 4K based
              p_ctx->args_encoder_write[frame_chunk_counter].frame_chunk_index = ((frame_chunk_counter*p_ctx->max_nvme_io_size) / 
                                                                                  FRAME_CHUNK_INDEX_SIZE);
              ...
              // if queue if full then we need to jump out this enqueu loop to check 
              // if there is worker availe and then trigger the worker with the argument in the queue
              while(is_worker_q_full(&(p_ctx->encoder_write_workerqueue)))
              {
        ...
        last_frame_chunk_send.data_len = ((frame_remain_bytes) < (p_ctx->max_nvme_io_size)) ?
                                          frame_remain_bytes :((p_ctx->max_nvme_io_size));
        last_frame_chunk_send.p_data = (ishwframe) ? (uint8_t*)p_frame->p_data[3] : p_data;
        ...
        WRITE_INSTANCE_SET_DW12_ISHWDESC(last_frame_chunk_send.p_ni_nvme_cmd->cdw12, (ishwframe ? NI_CODEC_HW_ENABLE : 0));
        ...
        while(ret_last != 0 ){
          ni_log(NI_LOG_TRACE, "encoder_wwrite opcode %d, handle %d, p_ni_nvme_cmd dw10:0x%8x dw11:0x%8x dw15:0x%8x,"
                 "data_len %d, pdataoff %p \n",
          ...
          {
            last_frame_chunk_send.data_len = ((last_frame_chunk_send.data_len / NI_MEM_PAGE_ALIGNMENT) *
                                              NI_MEM_PAGE_ALIGNMENT) + NI_MEM_PAGE_ALIGNMENT;
          }
#ifdef XCODER_OLD_NVME_DRIVER_ENABLED
...
enc_read_query:
  ...
  while (1)
  {
    ...
    else if (0 == inst_info.rd_buf_avail_size)
    {
      ...
      {
        retval = ni_query_stream_info(p_ctx, NI_DEVICE_TYPE_ENCODER, &data, ishwframe);
        CHECK_ERR_RC(p_ctx, retval, nvme_admin_cmd_xcoder_query,
                     p_ctx->device_type, p_ctx->hw_id,
                     &(p_ctx->session_id));
        CHECK_VPU_RECOVERY(retval);
...
#ifdef XCODER_IO_RW_ENABLED
  ui32LBA = READ_INSTANCE_R(ishwframe, p_ctx->session_id, NI_DEVICE_TYPE_ENCODER);
  ...
  if (!one_chunk_flag)
  {
    ...
    {
      ni_log(NI_LOG_TRACE, "rpkt_chunk_counter %d total_num_of_pkt_chunks %d to_read_size %u "
             "p_ctx->max_nvme_io_size %d pkt_remain_bytes %d\n",
      ...
      p_ctx->args_encoder_read[pkt_chunk_counter].data_len = (pkt_remain_bytes < p_ctx->max_nvme_io_size) ?
                                                             (pkt_remain_bytes) : (p_ctx->max_nvme_io_size);
      ...
      p_ctx->args_encoder_read[pkt_chunk_counter].frame_chunk_index = ((pkt_chunk_counter*p_ctx->max_nvme_io_size) /
                                                                       FRAME_CHUNK_INDEX_SIZE);
  ...
  last_pkt_chunk_read.data_len = (pkt_remain_bytes < p_ctx->max_nvme_io_size) ?
                                 (pkt_remain_bytes) : (p_ctx->max_nvme_io_size);
  ...
  while (ret_last != 0)
  {
    ni_log(NI_LOG_TRACE, "encoder read opcode %d, handle %d, p_ni_nvme_cmd dw10:0x%8x dw11:0x%8x dw15:0x%8x,"
           "  data_len %d, pdataoff %p \n",
    ...
    if (last_pkt_chunk_read.data_len % NI_MEM_PAGE_ALIGNMENT)
    {
      last_pkt_chunk_read.data_len = ((last_pkt_chunk_read.data_len / NI_MEM_PAGE_ALIGNMENT) * NI_MEM_PAGE_ALIGNMENT) +
  ...
  p_packet->avg_frame_qp = p_meta->avg_frame_qp;
  p_packet->recycle_index = p_meta->recycle_index;
  ni_debug_print("RECYCLE INDEX = %d!!! \n", p_meta->recycle_index);
  p_ctx->codec_total_ticks += p_meta->frame_cycle;
  ...
  if (size > 0)
  {
    ...
    {
      pthread_mutex_lock(p_ctx->dts_queue_mutex);
      if (ni_timestamp_get_with_threshold(p_ctx->dts_queue, 0, (int64_t*)& p_packet->dts, 0,
                                          encq_count % 500 == 0, p_ctx->buffer_pool) != NI_RETCODE_SUCCESS)
      {
  ...
  ni_log(NI_LOG_TRACE, "ni_encoder_session_read(): %d.%d p_packet->start_of_stream=%d, end_of_stream=%d, "
         "video_width=%d, video_height=%d, dts=0x%08x 0x%08x, pts=0x%08x 0x%08x, type=%u, avg_frame_qp=%u\n",
  ...
  if ((p_packet->dts != NI_NOPTS_VALUE) && (p_ctx->frame_time_q != NULL)) {
    ...
    ni_log(NI_LOG_INFO, "DTS:%lld,DELTA:%lu,eLAT:%lu;\n",
           p_packet->dts,
           abs_time_ns - p_ctx->prev_read_frame_time,
           ni_lat_meas_q_check_latency(p_ctx->frame_time_q, abs_time_ns, p_packet->dts));
    p_ctx->prev_read_frame_time = abs_time_ns;
  }
#endif
...
int ni_encoder_session_query(ni_session_context_t* p_ctx)
{
  ...
  if (NI_RETCODE_SUCCESS == retval)
  {
    ...
    ni_log(NI_LOG_TRACE, "ni_encoder_session_query current_load:%d fw_model_load:%d "
           "fw_video_mem_usage:%d active_contexts %d\n",
           p_ctx->load_query.current_load, p_ctx->load_query.fw_model_load,
           p_ctx->load_query.fw_video_mem_usage, p_ctx->load_query.total_contexts);
  }
                            ...
                            ni_instance_mgr_general_status_t* p_gen_status)
{
  ...
  int retval = NI_RETCODE_SUCCESS;
  uint32_t ui32LBA = 0;
  uint32_t dataLen = ((sizeof(ni_instance_mgr_general_status_t) + (NI_MEM_PAGE_ALIGNMENT - 1)) / NI_MEM_PAGE_ALIGNMENT) *
                      NI_MEM_PAGE_ALIGNMENT;
                                  ...
                                  bool is_hw)
{
  ...
  ni_retcode_t retval = NI_RETCODE_SUCCESS;
  uint32_t ui32LBA = 0;
  uint32_t dataLen = ((sizeof(ni_instance_mgr_stream_info_t) + (NI_MEM_PAGE_ALIGNMENT - 1)) / NI_MEM_PAGE_ALIGNMENT) *
                     NI_MEM_PAGE_ALIGNMENT;
                                  ...
                                  int opcode)
{
  ...
  void* p_buffer = NULL;
  uint32_t ui32LBA = 0;
  uint32_t dataLen = ((sizeof(ni_instance_status_info_t) + (NI_MEM_PAGE_ALIGNMENT - 1)) / NI_MEM_PAGE_ALIGNMENT) *
                     NI_MEM_PAGE_ALIGNMENT;
  ...
  p_status_info->sess_timestamp = ni_htonll(p_status_info->sess_timestamp);
  // session statistics
  ...
  if (NI_NO_CHECK_TS_NVME_CMD_OP == opcode)
  {
           ...
           (ni_xcoder_resource_recovery != p_status_info->inst_err_no)) // if VPU recovery, the session timestamp will be reset.
  {
    ...
    ni_log(NI_LOG_ERROR, "instance id invalid:%u, timestamp:%" PRIu64 ", query timestamp:%" PRIu64 "\n",
           p_ctx->session_id,
                 ...
                 ni_instance_mgr_stream_complete_t* p_stream_complete)
{
  ...
  ni_retcode_t retval = NI_RETCODE_SUCCESS;
  uint32_t ui32LBA = 0;
  uint32_t dataLen = ((sizeof(ni_instance_mgr_stream_complete_t) + (NI_MEM_PAGE_ALIGNMENT - 1)) / NI_MEM_PAGE_ALIGNMENT) *
                     NI_MEM_PAGE_ALIGNMENT;
                                        ...
                                        bool is_hw)
{
  ...
  {
    QUERY_INSTANCE_SET_DW11_INSTANCE_BUF_INFO(
      cmd.cdw11, nvme_query_xcoder_instance_upload_idx, device_type);
  }
  else
  {
    ni_log(NI_LOG_ERROR, "ERROR: Unknown query type, return\n");
 ...
 *            failure
 ******************************************************************************/
ni_retcode_t ni_config_instance_rw(ni_session_context_t* p_ctx,
                                  ni_inst_config_rw_type_t rw_type,
                                  uint8_t enable,
                                  uint8_t hw_action,
                                  uint16_t frame_id)
{
  ...
  void * p_buffer = NULL;
  uint32_t buffer_size = 0;
  ni_inst_config_rw_t * rw_config = NULL;

  ni_log(NI_LOG_TRACE, "ni_config_instance_rw(): enter\n");
  ...
  buffer_size = ((sizeof(ni_inst_config_rw_t) + (NI_MEM_PAGE_ALIGNMENT - 1)) / NI_MEM_PAGE_ALIGNMENT) * NI_MEM_PAGE_ALIGNMENT;
  ...
  rw_config = (ni_inst_config_rw_t *)p_buffer;
  ...
  {
    case INST_READ_CONFIG:
      ui32LBA = CONFIG_INSTANCE_FrameIdx_W(p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
      break;
    case INST_WRITE_CONFIG:
  ...
  {
    ni_log(NI_LOG_ERROR, " ni_config_instance_rw(): NVME command Failed\n");
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
    LRETURN;
  }
#else
  CONFIG_SESSION_SET_DW10_SESSION_ID(cmd.cdw10, p_ctx->session_id);
  CONFIG_SESSION_SET_DW15_SIZE(cmd.cdw15, sizeof(ni_inst_config_rw_t));

  switch(rw_type)
    {
    case SESSION_READ_CONFIG:
      CONFIG_SESSION_SET_DW11_SUBTYPE(cmd.cdw11, nvme_config_xcoder_config_session_read);
      break;
    case SESSION_WRITE_CONFIG:
      CONFIG_SESSION_SET_DW11_SUBTYPE(cmd.cdw11, nvme_config_xcoder_config_session_write);
      break;
    default:
      ni_log(NI_LOG_ERROR, "ERROR: Unknown rw type, return\n");
      retval = NI_RETCODE_INVALID_PARAM;
  ...
  if (ni_nvme_send_admin_cmd(nvme_admin_cmd_xcoder_config, p_ctx->device_handle, &cmd, buffer_size, p_buffer, &nvme_result))
  {
    ni_log(NI_LOG_ERROR, " ni_config_session_rw(): NVME command Failed\n");
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
  }
#endif
  ...
  ni_log(NI_LOG_TRACE, "ni_config_instance_rw(): exit\n");
 ...
 *
 *  \param   ni_session_context_t p_ctx - xcoder Context
 *  \param   ni_device_type_t device_type - xcoder type Encoder, Decoder or Uploader
 *
...
ni_retcode_t ni_config_instance_sos(ni_session_context_t* p_ctx, ni_device_type_t device_type)
{
  ...
  ni_retcode_t retval = NI_RETCODE_SUCCESS;
  uint32_t ui32LBA = 0;
  bool is_hw = (NI_DEVICE_TYPE_UPLOAD == device_type);
  ...
  if (! (NI_DEVICE_TYPE_DECODER == device_type ||
         NI_DEVICE_TYPE_ENCODER == device_type ||
         NI_DEVICE_TYPE_UPLOAD == device_type))
  {
    ni_log(NI_LOG_TRACE, "ERROR: Unknown device type, return\n");
    retval = NI_RETCODE_INVALID_PARAM;
    LRETURN;
  ...
  if (ni_nvme_send_write_cmd(p_ctx->blk_io_handle, p_ctx->event_handle, p_ctx->p_all_zero_buf,
...
ni_retcode_t ni_config_instance_set_encoder_params(ni_session_context_t* p_ctx)
{
  ...
  if (NI_RETCODE_SUCCESS !=ni_validate_custom_template(p_ctx, p_encoder_config, p_ctx->p_session_config,
  ...
  p_cfg->ui8hwframes = ni_htonl(p_cfg->ui8hwframes);
  ...
  if (NI_RETCODE_SUCCESS != retval)
  {
    ni_log(NI_LOG_TRACE, "ERROR: ni_nvme_send_admin_cmd failed: blk_io_handle: %" PRIx64 ", "
           "hw_id, %u, xcoder_inst_id: %d\n", (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id, p_ctx->session_id);
    //Close the session since we can't configure it
    retval = ni_encoder_session_close(p_ctx, 0);
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_TRACE, "ERROR: ni_encoder_session_close failed: blk_io_handle: %" PRIx64 ", "
             "hw_id, %u, xcoder_inst_id: %d\n", (int64_t)p_ctx->blk_io_handle, p_ctx->hw_id, p_ctx->session_id);
    }
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
  }
#else
  if (ni_nvme_send_admin_cmd(nvme_admin_cmd_xcoder_config, p_ctx->device_handle, &cmd, buffer_size,
                             p_encoder_config, &nvme_result))
  {
    ni_log(NI_LOG_TRACE, "ERROR: ni_nvme_send_admin_cmd failed: device_handle: %" PRIx64 ", "
           "hw_id, %u, xcoder_inst_id: %d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
    //Close the session since we can't configure it
    retval = ni_encoder_session_close(p_ctx, 0);
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_TRACE, "ERROR: ni_encoder_session_close failed: device_handle: %" PRIx64 ", "
             "hw_id, %u, xcoder_inst_id: %d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
    }
...
#else
  if (ni_nvme_send_admin_cmd(nvme_admin_cmd_xcoder_config, p_ctx->device_handle, &cmd, buffer_size,
                             p_encoder_config, &p_ctx->session_id))
  {
    ni_log(NI_LOG_TRACE, "ERROR: ni_nvme_send_admin_cmd failed: device_handle: %" PRIx64 ", "
           "hw_id, %d, xcoder_inst_id: %d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
    //Close the session since we can't configure it as per Farhan
...
int ni_create_frame(ni_frame_t* p_frame, uint32_t read_length, uint64_t* p_frame_offset, bool is_hw_frame)
{
  ...
  *p_frame_offset = 0;
  ...
  int metadata_size = NI_FW_META_DATA_SZ - (1 - is_hw_frame) * sizeof(ni_hwframe_surface_t);
  unsigned int video_data_size = p_frame->data_len[0] +
  p_frame->data_len[1] + p_frame->data_len[2] + ((is_hw_frame) ? p_frame->data_len[3] : 0);
  ni_log(NI_LOG_TRACE, "ni_create_frame rx_size = %d video_data_size = %d metadataSize = %d\n",
         rx_size, video_data_size, metadata_size);
  ...
  if (rx_size > video_data_size)
  {
    ...
    *p_frame_offset = p_meta->metadata_common.ui64_data.frame_offset;
    ...
    ni_log(NI_LOG_TRACE, "ni_create_frame: is_hw_frame=%d, [metadata] cropRight=%u, cropLeft=%u, "
           ...
           p_meta->metadata_common.frame_height, p_meta->metadata_common.bit_depth, p_frame->ni_pict_type,
...
void ni_set_default_template(ni_session_context_t* p_ctx, ni_encoder_config_t* p_config)
{
  ...
  p_config->niParamT408.cu_size_mode = NI_DEFAULT_CU_SIZE_MODE;
                                         ...
                                         uint32_t max_err_len)
{
        ...
        (p_cfg->niParamT408.intra_period % 8) != 0)))
  {
    ...
    param_ret = NI_RETCODE_PARAM_GOP_INTRA_INCOMPATIBLE;
    LRETURN;
  }
  ...
  if (STD_AVC == p_cfg->ui8bitstreamFormat)
  {
    switch (p_cfg->niParamT408.decoding_refresh_type)
    {
      ...
      default:
      {
        ni_log(NI_LOG_TRACE, "ERROR: ni_validate_custom_template() unknown "
               "value for niParamT408.decoding_refresh_type: %d\n",
               p_cfg->niParamT408.decoding_refresh_type);
      }
    }
  }
 ...
 *  \return
 *******************************************************************************/
void ni_params_print(ni_encoder_params_t* const p_codec_params, ni_device_type_t device_type)
{
  if (!p_codec_params)
  {
    return;
  ...
  if (NI_DEVICE_TYPE_DECODER == device_type)
  {
    ni_decoder_input_params_t* p_dec = &p_codec_params->dec_input_params;
  ...
  else if (NI_DEVICE_TYPE_ENCODER)
  {
    ni_h265_encoder_params_t* p_enc = &p_codec_params->hevc_enc_params;
  ...
  else if (NI_DEVICE_TYPE_UPLOAD == device_type)
  {
    ni_decoder_input_params_t* p_dec = &p_codec_params->dec_input_params;
...
*          negative value like NI_RETCODE_FAILURE in ni_retcode_t on failure
*  \return
*******************************************************************************/
ni_retcode_t ni_uploader_session_open(ni_session_context_t* p_ctx)
{
  ...
  ni_log(NI_LOG_TRACE, "ni_uploader_session_open(): enter\n");

  if (!p_ctx)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_uploader_session_open(): passed parameters are null!, return\n");
    retval = NI_RETCODE_INVALID_PARAM;
    LRETURN;
  }
  //pthread_mutex_alloc_and_init(&(p_ctx->dts_queue_mutex));

#ifndef XCODER_IO_RW_ENABLED
  pthread_mutex_alloc_and_init(&(p_ctx->decoder_read_mutex));
  pthread_mutex_alloc_and_init(&(p_ctx->decoder_read_mutex_len));
  pthread_mutex_alloc_and_init(&(p_ctx->encoder_write_mutex));
  pthread_mutex_alloc_and_init(&(p_ctx->encoder_write_mutex_len));
  pthread_cond_alloc_and_init(&(p_ctx->decoder_read_cond));
  pthread_cond_alloc_and_init(&(p_ctx->encoder_write_cond));
  sem_alloc_and_init(&(p_ctx->decoder_read_semaphore));
  sem_alloc_and_init(&(p_ctx->encoder_write_semaphore));
  worker_q_init(&(p_ctx->decoder_read_workerqueue), p_ctx->worker_queue_decoder_read);
  ...
  for (counter = 0; counter< MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    pthread_create(&(p_ctx->ThreadID_decoder_read[counter]), NULL, workerthread_decoder_read, p_ctx);
    pthread_create(&(p_ctx->ThreadID_encoder_write[counter]), NULL, workerthread_encoder_write, (void*)p_ctx);
  }
#endif
  //Create the session if the create session flag is set
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    p_ctx->device_type = NI_DEVICE_TYPE_DECODER;//NI_DEVICE_TYPE_UPLOAD;
    p_ctx->pts_table = NULL;
    p_ctx->dts_queue = NULL;
    p_ctx->p_leftover = NULL;
    p_ctx->buffer_pool = NULL;
    ...
    if(posix_memalign(&p_ctx->p_all_zero_buf, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
    {
      ni_log(NI_LOG_ERROR, "ERROR: ni_uploader_session_open() alloc all zero buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    memset(p_ctx->p_all_zero_buf, 0, NI_DATA_BUFFER_LEN);
    ...
    if(posix_memalign(&p_ctx->p_dec_packet_inf_buf, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_open(): alloc decoder packet info buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    memset(p_ctx->p_dec_packet_inf_buf, 0, NI_DATA_BUFFER_LEN);
    ...
    if(posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
    {
      ni_log(NI_LOG_ERROR, "ERROR: ni_uploader_session_open() alloc data buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    memset(p_buffer, 0, NI_DATA_BUFFER_LEN);
    ...
    // Get session ID
    ui32LBA = OPEN_GET_SID_R(1, NI_DEVICE_TYPE_DECODER);
    retval = ni_nvme_send_read_cmd(p_ctx->blk_io_handle, p_ctx->event_handle,
                                   p_buffer, NI_DATA_BUFFER_LEN, ui32LBA);
    ...
    if (NI_INVALID_SESSION_ID == p_ctx->session_id)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_uploader_session_open(): p_ctx->device_handle=%" PRIx64 ", p_ctx->hw_id=%d, p_ctx->session_id=%d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
      ni_decoder_session_close(p_ctx, 0);
      retval = NI_RETCODE_ERROR_INVALID_SESSION;
      LRETURN;
    }
    ...
    session_info.codec_format = ni_htonl(p_ctx->codec_format);
    session_info.model_load = ni_htonl(model_load);
    session_info.hw_desc_mode = ni_htonl(NI_CODEC_HW_UPLOAD);

    memcpy(p_buffer, &session_info, sizeof(ni_decoder_session_open_info_t));
    ui32LBA = OPEN_SESSION_W(1, p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
    retval = ni_nvme_send_write_cmd(p_ctx->blk_io_handle, p_ctx->event_handle,
                                    p_buffer, NI_DATA_BUFFER_LEN, ui32LBA);
    CHECK_ERR_RC(p_ctx, retval, NI_NO_CHECK_TS_NVME_CMD_OP,
                 p_ctx->device_type, p_ctx->hw_id,
...
#if 0 // no parametes to set this when uploading. use the default timeout now.
    //Send keep alive timeout Info
    uint64_t keep_alive_timeout = p_ctx->keep_alive_timeout * 1000000;  //send us to FW
    memset(p_buffer, 0, NI_DATA_BUFFER_LEN);
    memcpy(p_buffer, &keep_alive_timeout, sizeof(keep_alive_timeout));
...
#endif
    ni_log(NI_LOG_TRACE, "Uploader open session ID:0x%x\n",p_ctx->session_id);
    aligned_free(p_buffer);
#else //XCODER_IO_RW_ENABLED

    CREATE_SESSION_SET_DW10_SUBTYPE(cmd.cdw10);
    CREATE_SESSION_SET_DW11_INSTANCE(cmd.cdw11, NI_DEVICE_TYPE_ENCODER); //need to use encode instance 
    CREATE_SESSION_SET_DW12_DEC_CID(cmd.cdw12, p_ctx->codec_format);

    if (ni_nvme_send_admin_cmd(nvme_admin_cmd_xcoder_open, p_ctx->device_handle, &cmd, 0, NULL, &(p_ctx->session_id)))
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_uploader_session_open(): p_ctx->device_handle=%" PRIx64 " , p_ctx->hw_id=%d, p_ctx->session_id=%d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    ...
    //retval = ni_config_instance_set_uploader_params(p_ctx);
    //if (NI_RETCODE_SUCCESS != retval)
    //{
    //    ni_debug_print("ERROR: calling ni_config_instance_set_encoder_params(): p_ctx->device_handle=%" PRIx64 ", p_ctx->hw_id=%d, p_ctx->session_id=%d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
    //    ni_encoder_session_close(p_ctx, 0);
    //    retval = NI_RETCODE_ERROR_INVALID_SESSION;
    //    LRETURN;
    //}
    ...
    ni_log(NI_LOG_TRACE, "ni_uploader_session_open(): p_ctx->device_handle=%" PRIx64 ", p_ctx->hw_id=%d, p_ctx->session_id=%d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
#endif //XCODER_IO_RW_ENABLED
  }
...
//  ni_timestamp_init(p_ctx, (ni_timestamp_table_t * *) & (p_ctx->dts_queue), "dec_dts");
  ...
  ni_log(NI_LOG_TRACE, "ni_uploader_session_open(): p_ctx->device_handle=%" PRIx64 ", p_ctx->hw_id=%d, "
         "p_ctx->session_id=%d\n",
         (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
  ...
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_uploader_session_open(): Cannot open dump file\n");
  }
#endif

  END;

  ni_log(NI_LOG_TRACE, "ni_uploader_session_open(): exit\n");
  return retval;
}
...
*
*  \param  ni_session_context_t p_ctx - source xcoder Context
*  \param  ni_session_context_t p_ctx - destination xcoder Context
*
*  \return NI_RETCODE_SUCCESS on success,
*          NI_RETCODE_INVALID_PARAM on failure
*******************************************************************************/
...
#ifndef XCODER_IO_RW_ENABLED
  int counter = 0;
  pthread_mutex_alloc_and_init(&dst_p_ctx->decoder_read_mutex);
  pthread_mutex_alloc_and_init(&dst_p_ctx->decoder_read_mutex_len);
  pthread_cond_alloc_and_init(&dst_p_ctx->decoder_read_cond);
  sem_alloc_and_init(&dst_p_ctx->decoder_read_semaphore);
  worker_q_init(&(dst_p_ctx->decoder_read_workerqueue), dst_p_ctx->worker_queue_decoder_read);
  // Here we create a worker thread pool

  for (counter = 0; counter< MAX_NUM_OF_THREADS_PER_FRAME; counter++)
  {
    pthread_create(&(dst_p_ctx->ThreadID_decoder_read[counter]), NULL, workerthread_decoder_read, dst_p_ctx);
  }
#endif
...
*
*  \param  ni_session_context_t p_ctx - xcoder Context
*  \param  ni_frame_t* p_frame - xcoder frame
*
*  \return NI_RETCODE_SUCCESS on success,
*          negative value like NI_RETCODE_FAILURE in ni_retcode_t on failure
*******************************************************************************/
                              ...
                              ni_frame_t* p_frame)
{
  ...
  ni_log(NI_LOG_TRACE, "ni_hwupload_session_write(): enter\n");
  ...
  ni_log(NI_LOG_TRACE, "ni_hwupload_session_write(): frame size bytes =%d  %d is metadata!\n",
         frame_size_bytes, 0);
  
  
  retval = ni_query_instance_buf_info(p_ctx, INST_BUF_INFO_RW_WRITE,
                                      NI_DEVICE_TYPE_DECODER, &buf_info, 1);
  CHECK_ERR_RC(p_ctx, retval, nvme_admin_cmd_xcoder_query,
               p_ctx->device_type, p_ctx->hw_id,
               &(p_ctx->session_id));
  ...
  if (!p_ctx->frame_num)
  {
    ...
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_TRACE, "ERROR ni_hwupload_session_write(): Failed to send SOS.\n");
      LRETURN;
    }
  }
  ...
  if (p_frame->p_data)
  {
    ...
    retval = ni_nvme_send_write_cmd(
      p_ctx->blk_io_handle, p_ctx->event_handle, p_ctx->p_dec_packet_inf_buf,
      NI_DATA_BUFFER_LEN, CONFIG_INSTANCE_SetPktSize_W(1, p_ctx->session_id,
                                                       NI_DEVICE_TYPE_DECODER));
    CHECK_ERR_RC(p_ctx, retval, nvme_cmd_xcoder_write,
                 p_ctx->device_type, p_ctx->hw_id, &(p_ctx->session_id));
    CHECK_VPU_RECOVERY(retval);
    ...
    uint32_t ui32LBA = WRITE_INSTANCE_W(1, p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
    ni_log(NI_LOG_TRACE, "hwupload session write: p_data = %p, p_frame->buffer_size = %u, "
    ...
    if (retval < 0)
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_hwupload_session_write(): nvme command failed\n");
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
    ...
    // while (frame_size_bytes > 0)
    {
        ...
        (tx_size != p_ctx->max_nvme_io_size))
      {
        ...
        if (tx_size % NI_MEM_PAGE_ALIGNMENT)
        {
          ni_log(NI_LOG_TRACE, "WARN: before calc tx_size = %u\n", tx_size);
          aligned_tx_size = ((tx_size / NI_MEM_PAGE_ALIGNMENT) * NI_MEM_PAGE_ALIGNMENT) + NI_MEM_PAGE_ALIGNMENT;
          //frame_size_bytes = aligned_tx_size;
          ni_log(NI_LOG_TRACE, "WARN: after calc tx_size = %u aligned_tx_size %d frame_size_bytes changed to this\n", tx_size, aligned_tx_size);
        }
      }
      ...
      int32_t frame_remain_bytes = frame_size_bytes;
      p_ctx->hw_action = NI_CODEC_HW_UPLOAD;
      if (!one_chunk_flag) {
        for (; frame_chunk_counter < total_num_of_chunks - 1; frame_chunk_counter++)
        {
          ni_log(NI_LOG_TRACE, "frame chunk counter %d  opcode %d remain %d dl %d\n", \
      ...
      WRITE_INSTANCE_SET_DW11_INSTANCE(last_frame_chunk_send.p_ni_nvme_cmd->cdw11, NI_DEVICE_TYPE_ENCODER);
      WRITE_INSTANCE_SET_DW11_PAGEOFFSET(last_frame_chunk_send.p_ni_nvme_cmd->cdw11, last_frame_chunk_send.frame_chunk_index);  // set page index based on 4k
      WRITE_INSTANCE_SET_DW12_ISHWDESC(last_frame_chunk_send.p_ni_nvme_cmd->cdw12, NI_CODEC_HW_UPLOAD);
      WRITE_INSTANCE_SET_DW15_SIZE(last_frame_chunk_send.p_ni_nvme_cmd->cdw15, last_frame_chunk_send.data_len);
      ...
      uint32_t data_len_local = last_frame_chunk_send.data_len;
      while (ret_last != 0) {
        ni_log(NI_LOG_TRACE, "ni_hwupload_session_write opcode %d, handle %d, p_ni_nvme_cmd dw10:0x%8x dw11:0x%8x dw12:0x%8x dw15:0x%8x,  data_len %d, pdataoff %p \n", \
          last_frame_chunk_send.opcode, \
          last_frame_chunk_send.handle, \
          last_frame_chunk_send.p_ni_nvme_cmd->cdw10, \
          last_frame_chunk_send.p_ni_nvme_cmd->cdw11, \
          last_frame_chunk_send.p_ni_nvme_cmd->cdw12, \
          last_frame_chunk_send.p_ni_nvme_cmd->cdw15, \
          last_frame_chunk_send.data_len, \
          ((last_frame_chunk_send.p_data) + (last_frame_chunk_send.frame_chunk_index)*FRAME_CHUNK_INDEX_SIZE)
        );
        ...
        if (ret_last != NI_RETCODE_SUCCESS)
        {
          ni_log(NI_LOG_ERROR, "ni_hwupload_session_write failed retry\n");
          *(last_frame_chunk_send.p_result) = 0;
          retry_count++;
          ni_usleep(100);
          if (retry_count == 6000)
          {
            ni_log(NI_LOG_ERROR, "Something is wrong as the encoder write command retried 6000 times, now assert\n");
            assert(false);
          }
          continue;
        }
...
#ifdef XCODER_DUMP_ENABLED
      fwrite(p_data, tx_size, 1, p_ctx->p_dump[0]);
#endif
      ni_log(NI_LOG_TRACE, "ni_hwupload_session_write:  retval=%d\n", retval);
    }
...
#ifdef XCODER_TIMESTAMP_ENC_ENABLED
#if 0 // for debug purpose
    //  printf("Enc -> pts %lld (dts %lld) \n", p_frame->pts, p_frame->dts);
#endif
#endif
  ...
  ni_log(NI_LOG_TRACE, "ni_hwupload_session_write(): exit\n");
  return retval;
}
...
*          which stored in HW through ni_hwupload_session_write().
*
*  \param  ni_session_context_t p_ctx - xcoder Context
*  \param  ni_hwframe_surface_t* hwdesc - xcoder hardware descriptor
*
*  \return NI_RETCODE_SUCCESS on success,
*          negative value like NI_RETCODE_FAILURE in ni_retcode_t on failure
*******************************************************************************/
int ni_hwupload_session_read_hwdesc(ni_session_context_t* p_ctx,
                                    ni_hwframe_surface_t* hwdesc)
{
  int retval = NI_RETCODE_SUCCESS;
  ni_instance_buf_info_t hwdesc_info = { 0 };
  ni_log(NI_LOG_TRACE, "ni_hwupload_session_read_hwdesc(): enter\n");
  ...
  retval = ni_query_instance_buf_info(p_ctx, INST_BUF_INFO_RW_UPLOAD,
    NI_DEVICE_TYPE_DECODER, &hwdesc_info, 1);
  CHECK_ERR_RC(p_ctx, retval, nvme_admin_cmd_xcoder_query,
    p_ctx->device_type, p_ctx->hw_id,
    &(p_ctx->session_id));
  ...
  else
  {
    ...
    hwdesc->i8FrameIdx = hwdesc_info.hw_inst_ind.frame_index;
...
#else
    hwdesc->device_handle = p_ctx->device_handle;
#endif
...
*          The device handel is got from decoder or uploader,
...
*            failure
******************************************************************************/
ni_retcode_t ni_clear_instance_buf(ni_hwframe_surface_t* surface,
                                   ni_device_handle_t device_handle)
{
  ni_nvme_result_t nvme_result = 0;
  ni_nvme_command_t cmd = { 0 };
  ...
  ni_log(NI_LOG_TRACE, "ni_clear_instance_buf(): enter - device_handle %d\n", device_handle);

  if (NI_INVALID_SESSION_ID == surface->ui16SessionID)
  {
    ...
    if(posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), NI_DATA_BUFFER_LEN))
    {
      ni_log(NI_LOG_ERROR, "ERROR: ni_clear_instance_buf() alloc data buffer failed\n");
      retval = NI_RETCODE_ERROR_MEM_ALOC;
      LRETURN;
    }
    memset(p_buffer, 0, NI_DATA_BUFFER_LEN);

    ((ni_recycle_buffer_t *)p_buffer)->i8FrameIdx = surface->i8FrameIdx;
    ((ni_recycle_buffer_t *)p_buffer)->i8InstID = surface->i8InstID;
    //maybe just set 13 aqs inst id again?
    ni_log(NI_LOG_TRACE, "ni_clear_instance_buf():i8FrameIdx = %d, i8InstID = %d\n",
           surface->i8FrameIdx, surface->i8InstID);
    
    ui32LBA = CONFIG_INSTANCE_RecycleBuf_W(surface->ui16SessionID,NI_DEVICE_TYPE_DECODER);
    retval = ni_nvme_send_write_cmd(device_handle, NI_INVALID_DEVICE_HANDLE,
                                    p_buffer, NI_DATA_BUFFER_LEN, ui32LBA);
    //Cannot check sessio stats here since this isn't a session command.
    if(retval < 0)
    {
      ni_log(NI_LOG_ERROR, " ni_clear_instance_buf(): NVME command Failed\n");
      retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
      LRETURN;
    }
  ...
  RECYCLE_BUFFER_SET_DW10_BUFID(cmd.cdw10, (uint32_t)surface->ui16FrameIdx);

  //maybe just set 13 aqs inst id again?
  ni_log(NI_LOG_TRACE, "ni_clear_instance_buf():cdw10=0x%x FID = %d\n",
   cmd.cdw10, surface->ui16FrameIdx);

  if (ni_nvme_send_admin_cmd(nvme_admin_cmd_xcoder_recycle_buffer, device_handle,
    &cmd, dataLen, p_buffer, &nvme_result))
  {
    ni_log(NI_LOG_ERROR, " ni_clear_instance_buf(): NVME command Failed\n");
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
  ...
  ni_log(NI_LOG_TRACE, "ni_clear_instance_buf(): exit\n");
...
*  \return
*******************************************************************************/
void ni_populate_serial_number(ni_serial_num_t* p_serial_num, void* p_data)
{
  ni_nvme_identity_t* p_id_data = (ni_nvme_identity_t*)p_data;

  if ((!p_serial_num) || (!p_data))
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_populate_device_capability_struct(): Null pointer parameters passed\n");
    LRETURN;
  }
                                          ...
                                          ni_frame_t* p_frame)
{
  ...
  ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc(): enter\n");
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_read_desc(): xcoder instance id < 0, return\n");
    retval = NI_RETCODE_ERROR_INVALID_SESSION;
    LRETURN;
  }
  ...
  while (1)
  {
    ...
    else if (buf_info.buf_avail_size == (metadata_hdr_size - sizeof(ni_hwframe_surface_t)))
    {
      ni_log(NI_LOG_TRACE, "Info only metadata hdr is available, seq change?\n");
    ...
    else if (buf_info.buf_avail_size < total_yuv_met_size)
    {
      ...
      if (p_ctx->ready_to_close)
      {
        ...
        retval = ni_query_stream_info(p_ctx, NI_DEVICE_TYPE_DECODER, &data, 1);
        CHECK_ERR_RC(p_ctx, retval, nvme_admin_cmd_xcoder_query,
          p_ctx->device_type, p_ctx->hw_id,
          &(p_ctx->session_id));
        CHECK_VPU_RECOVERY(retval);
    ...
    else
    {
      ...
      if (0 == p_ctx->active_video_width || 0 == p_ctx->active_video_height)
      {
        ...
        ni_log(NI_LOG_TRACE, "Info dec YUV, adjust frame size from %ux%u %dbits to %ux%u\n",
               p_ctx->active_bit_depth, p_frame->video_width, p_frame->video_height,
               p_ctx->active_video_width, p_ctx->active_video_height);

        retval = ni_frame_buffer_alloc(p_frame,
        ...
        ni_log(NI_LOG_TRACE, "Info dec buf size: %u YUV frame + meta-hdr size: %u "
          "available: %u\n", p_frame->buffer_size,
          totall_bytes_to_read, buf_info.buf_avail_size);
      }
      break;
    }
  ...
  unsigned int bytes_read_so_far = 0;
#ifdef XCODER_IO_RW_ENABLED
  ni_log(NI_LOG_TRACE, "totall_bytes_to_read %d max_nvme_io_size %d ylen %d cr len %d cb len %d hwdes len %d hdr %d\n", \
         totall_bytes_to_read, \
         p_ctx->max_nvme_io_size, \
         p_frame->data_len[0], \
         p_frame->data_len[1], \
  ...
  if (totall_bytes_to_read == metadata_hdr_size) // metadata alone, seqchange?
  {
    buf_info.buf_avail_size = buf_info.buf_avail_size + sizeof(ni_hwframe_surface_t);
  }
  else
  {
    buf_info.buf_avail_size = buf_info.buf_avail_size + 2 * sizeof(ni_hwframe_surface_t) -
                              (p_ctx->active_video_width * p_ctx->active_video_height * 3 / 2) * p_ctx->bit_depth_factor;
  }
  ni_log(NI_LOG_TRACE, "buf_avail_size = %d \n", buf_info.buf_avail_size);
  if (buf_info.buf_avail_size < totall_bytes_to_read)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_read() avaliable size(%u)"
           "less than needed (%u)\n",
           buf_info.buf_avail_size, totall_bytes_to_read);
    ni_assert(0);
  }

  read_size_bytes = buf_info.buf_avail_size;
  ui32LBA = READ_INSTANCE_R(1, p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
  if (read_size_bytes % NI_MEM_PAGE_ALIGNMENT)
  {
    read_size_bytes = ( (read_size_bytes / NI_MEM_PAGE_ALIGNMENT) * NI_MEM_PAGE_ALIGNMENT) + NI_MEM_PAGE_ALIGNMENT;
  }
  ...
  if (retval < 0)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_decoder_session_read_desc(): nvme command failed\n");
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
    LRETURN;
  }
  else
  {
    ...
    if (buf_info.buf_avail_size != metadata_hdr_size)
    {
      ...
      ni_hwframe_surface_t* p_data3 = (ni_hwframe_surface_t*)((uint8_t*)p_frame->p_buffer + p_frame->data_len[0]
      ...
      ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc(): i8FrameIdx:%d, %d\n",
             p_data3->i8FrameIdx,
             p_meta->hwdesc.i8FrameIdx);
      ...
      ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc:sei_size=%d device_handle=%d == hw_id=%d ses_id=%d\n",
             sei_size, p_data3->device_handle, p_ctx->hw_id, p_data3->ui16SessionID);
      ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: ui16FrameIdx=%d, bit_depth(in data3)=%d, bit_depth(in meta)=%d\n",
             p_data3->i8FrameIdx, p_data3->bit_depth, p_meta->hwdesc.bit_depth);
      ...
      {
        ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: sequence change first frame, set seq_change\n");
    ...
    totall_bytes_to_read = totall_bytes_to_read + sei_size;
    p_ctx->decoder_read_total_num_of_bytes = p_ctx->decoder_read_total_num_of_bytes + sei_size;
    ni_log(NI_LOG_TRACE, "decoder read desc success, retval %d totall_bytes_to_read include sei %d sei_size %d frame_cycle %d\n",
           retval, totall_bytes_to_read, sei_size, frame_cycle);
  }
  ...
  ni_log(NI_LOG_TRACE, "total_num_of_chunksdr %d totall_bytes_to_read %d max_nvme_io_size %d ylen %d cr len %d cb len %d hdr %d\n", \
    ...
    p_frame->data_len[2], \
    metadata_hdr_size);
  ni_log(NI_LOG_TRACE, "p_frame->data_len[3] = %d \n", p_frame->data_len[3]);
...

  {
    ...
    //    p_ctx->args_decoder_read[frame_chunk_counter].session_id = p_ctx->session_id;
    //    frame_remain_bytes = frame_remain_bytes - p_ctx->args_decoder_read[frame_chunk_counter].data_len;
    //    ni_debug_print("Decoder read frame_chunk_counter %d totall_bytes_to_read %d p_ctx->decoder_read_total_num_of_chunks %d chunk index %d frame_remain_bytes %d\n", \
    //      frame_chunk_counter, \
    //      totall_bytes_to_read, \
    //      p_ctx->decoder_read_total_num_of_chunks, \
    //      p_ctx->args_decoder_read[frame_chunk_counter].frame_chunk_index, \
    ...
    last_pkt_chunk_read.handle = p_ctx->blk_io_handle;
    last_pkt_chunk_read.p_ni_nvme_cmd = &(cmd[frame_chunk_counter]);
    last_pkt_chunk_read.data_len = frame_remain_bytes;// +(buf_info.buf_avail_size - totall_bytes_to_read);
    last_pkt_chunk_read.p_data = p_data_buffer;
    last_pkt_chunk_read.p_result = &(nvme_result[frame_chunk_counter]);
    last_pkt_chunk_read.frame_chunk_index = ((frame_chunk_counter*p_ctx->max_nvme_io_size) / FRAME_CHUNK_INDEX_SIZE);
    last_pkt_chunk_read.frame_chunk_size = p_ctx->max_nvme_io_size;
    ...
    WRITE_INSTANCE_SET_DW12_ISHWDESC(last_pkt_chunk_read.p_ni_nvme_cmd->cdw12, NI_CODEC_HW_ENABLE);
    ...
    uint32_t data_len_local = last_pkt_chunk_read.data_len;
    while (ret_last != 0) {
      ni_log(NI_LOG_TRACE, "decoder_rread opcode %d, handle %d, p_ni_nvme_cmd dw10:0x%8x dw11:0x%8x dw12:0x%8x dw15:0x%8x,  data_len %d, pdataoff %p \n", \
        last_pkt_chunk_read.opcode, \
        last_pkt_chunk_read.handle, \
        last_pkt_chunk_read.p_ni_nvme_cmd->cdw10, \
        last_pkt_chunk_read.p_ni_nvme_cmd->cdw11, \
        last_pkt_chunk_read.p_ni_nvme_cmd->cdw12, \
        last_pkt_chunk_read.p_ni_nvme_cmd->cdw15, \
        last_pkt_chunk_read.data_len, \
        ((last_pkt_chunk_read.p_data) + (last_pkt_chunk_read.frame_chunk_index)*FRAME_CHUNK_INDEX_SIZE)
      );
      ...
      ret_last = ni_nvme_send_io_cmd(last_pkt_chunk_read.opcode, \
        ...
        last_pkt_chunk_read.p_result);
#endif
      CHECK_VPU_RECOVERY(ret_last);

      if (ret_last != NI_RETCODE_SUCCESS)
      {
        ni_log(NI_LOG_ERROR, "decoder read failed retry\n");
        *(last_pkt_chunk_read.p_result) = 0;
        retry_count++;
        ni_usleep(100);
        if (retry_count == 6000)
        {
          ni_log(NI_LOG_ERROR, "Something is wrong as the decoder read command retried 6000 times, now assert\n");
          assert(false);
        }
        continue;
      }
      else
      {
        ...
        ni_metadata_dec_frame_t* p_meta =
          (ni_metadata_dec_frame_t*)((uint8_t*)p_frame->p_buffer
            + p_frame->data_len[0] + p_frame->data_len[1]
            + p_frame->data_len[2] + p_frame->data_len[3]);
        memcpy(p_meta, p_frame->p_buffer + sizeof(ni_hwframe_surface_t) * NI_MAX_NUM_OF_DECODER_OUTPUTS, metadata_hdr_size);//shift metadata to end of triple output
        sei_size = p_meta->sei_size;
        ni_hwframe_surface_t* p_data3 =
          (ni_hwframe_surface_t*)((uint8_t*)p_frame->p_buffer
            + p_frame->data_len[0] + p_frame->data_len[1]
            + p_frame->data_len[2]);

        p_data3->device_handle = p_ctx->blk_io_handle; //Libxcoder knows the handle so overwrite here
        p_data3->ui16SessionID = (uint16_t)p_ctx->session_id;
        p_data3->src_cpu = (uint8_t)NI_DEVICE_TYPE_DECODER;

        ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc:sei_size=%d device_handle=%d == hw_id=%d ses_id=%d\n",
          sei_size, p_data3->device_handle, p_ctx->hw_id, p_data3->ui16SessionID);
        ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: ui16FrameIdx=%d NodeAddre=0x%x, planar=%d\n",
          p_data3->ui16FrameIdx, p_data3->ui32nodeAddress, p_data3->encoding_type);
        ...
        *(last_pkt_chunk_read.p_result) = data_len_local;
        totall_bytes_to_read = totall_bytes_to_read + sei_size;
        p_ctx->decoder_read_total_num_of_bytes = p_ctx->decoder_read_total_num_of_bytes + sei_size;
        ni_log(NI_LOG_TRACE, "decoder read success, size %d local %d totall_bytes_to_read include sei %d sei_size %d\n", *(last_pkt_chunk_read.p_result), data_len_local, totall_bytes_to_read, sei_size);
        break;
      }
    }
    p_ctx->decoder_read_processed_data_len += *(last_pkt_chunk_read.p_result);
    ...
    //if (!one_chunk_flag) {
    //  ni_debug_print("Totally_decoder_read enqueued %d\n", frame_chunk_counter);
    //  // After the queue is filled up, we release the worker threads
    //  // Here we need to pull untill the queue is empty,
    //  // There is a mutex to protect each queue check
    //  int num_of_thread_to_release = frame_chunk_counter - 1;
    ...
    //    // Looks like we have workers avaiable and there some work to do,
    //    // So compete to lock the mutex
    //    ni_debug_print("Release one thread for decoder read ntr %d\n", num_of_thread_to_release);
    //    sem_post(&p_ctx->decoder_read_semaphore);
    //    ni_debug_print("Finish release one thread decoder read num_of_thread_to_release %d\n", num_of_thread_to_release);
    //    num_of_thread_to_release--;
    //  }
    //
    //  // here we waif for one worker thread to notify the main thread that all the chunks had been sent out
    ...
    //  pthread_mutex_unlock(&(p_ctx->decoder_read_mutex_len));
    //
    //  ni_debug_print("No more decoder read ongoing progressed bytes %d total len %d frame num %lu\n", p_ctx->decoder_read_processed_data_len, totall_bytes_to_read, p_ctx->frame_num);
    //}
  }
#endif //XCODER_IO_RW_ENABLED
      ...
      totall_bytes_to_read != metadata_hdr_size)
  {
      ...
      if (p_ctx->is_first_frame)
      {
        ...
        else if((p_ctx->pts_offsets[0] != NI_NOPTS_VALUE) && (p_ctx->pkt_index != -1))
        {
          ni_metadata_dec_frame_t* p_metadata =
                  ...
                  p_frame->data_len[0] + p_frame->data_len[1] +
                  p_frame->data_len[2] + p_frame->data_len[3]);
          int num_fw_pkts = p_metadata->metadata_common.ui64_data.frame_offset / 512;
          ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: num_fw_pkts %u frame_offset %" PRIu64 " ni_create_frame\n",
                 num_fw_pkts, p_metadata->metadata_common.ui64_data.frame_offset);
          int idx = 0;
          int cumul = p_ctx->pkt_offsets_index[0];
          bool bFound = (num_fw_pkts >= cumul);
          while (cumul < num_fw_pkts) // look for pts index
          {
            ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: cumul %u\n", cumul);
            if (idx == NI_MAX_DEC_REJECT)
            {
              ni_log(NI_LOG_ERROR, "Invalid index computation oversizing NI_MAX_DEC_REJECT! \n");
              break;
            }
            else
            {
              idx ++;
              cumul += p_ctx->pkt_offsets_index[idx];
              ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc:  idx %d pkt_offsets_index[idx] %" PRIu64 "\n",
                     idx, p_ctx->pkt_offsets_index[idx]);
            }
          }
          ...
          if ((idx != NI_MAX_DEC_REJECT) && bFound)
          {
            ...
            if (idx)
              ...
              p_ctx->last_pts = p_frame->pts;
            p_ctx->last_dts = p_frame->dts;
            ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (first frame) idx %d last_dts %" PRIu64 " dts %llu "
                   "last_pts %" PRIu64 " pts %llu\n",
                   idx, p_ctx->last_dts, p_frame->dts, p_ctx->last_pts, p_frame->pts);
          }
          else if (p_ctx->session_run_state == SESSION_RUN_STATE_RESETTING)
          {
            ni_log(NI_LOG_TRACE, "xcoder_dec_receive(): session %u recovering and "
                           "adjusting ts.\n", p_ctx->session_id);
            p_frame->pts = p_ctx->pts_offsets[idx];
            p_ctx->last_pts = p_frame->pts;
            p_ctx->last_dts = p_frame->dts;
          ...
          else // use pts = 0 as offset
          {
            ...
            p_ctx->last_pts = 0;
            p_ctx->last_dts = p_frame->dts;
            ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (zero default) dts %llu pts %llu\n", p_frame->dts, p_frame->pts);
          }
        }
        else
        {
          ...
          p_ctx->last_pts = 0;
          p_ctx->last_dts = p_frame->dts;
          ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc:  (not bitstream) dts %llu pts %llu\n", p_frame->dts, p_frame->pts);
        }
      }
      else
      {
        ...
        p_ctx->last_pts = p_frame->pts;
        p_ctx->last_dts = p_frame->dts;
        ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc:  (!is_first_frame idx) last_dts %" PRIu64 " dts %llu pts_delta %" PRIu64 " "
               "last_pts %" PRIu64 " pts %llu\n", p_ctx->last_dts, p_frame->dts, pts_delta, p_ctx->last_pts, p_frame->pts);
      }
    }
    else
    {
      ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: frame_offset %" PRIu64 "\n", frame_offset);
      //ignore timestamps if bitstream
      if (p_frame->dts == NI_NOPTS_VALUE)
      {
        p_frame->pts = NI_NOPTS_VALUE;
        p_ctx->last_dts = p_ctx->last_pts = NI_NOPTS_VALUE;
        ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (dts NI_NOPTS_VALUE) dts %llu pts %llu\n", p_frame->dts, p_frame->pts);
      }
      else
      {
        if (p_ctx->is_first_frame)
        ...
        if (frame_offset == 0)
        {
          if (p_ctx->pts_offsets[0] == NI_NOPTS_VALUE)
          {
            p_frame->pts = NI_NOPTS_VALUE;
            p_ctx->last_dts = p_ctx->last_pts = NI_NOPTS_VALUE;
            ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (first frame pts NI_NOPTS_VALUE not bitstream) dts %llu pts %llu\n", p_frame->dts, p_frame->pts);
          }
          else
          {
            p_frame->pts = p_ctx->pts_offsets[0];
            p_ctx->last_pts = p_frame->pts;
            p_ctx->last_dts = p_frame->dts;
            ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (first frame) dts %llu pts %llu\n", p_frame->dts, p_frame->pts);
          }
        }
        else
        {
          for (i = 0 ; i < NI_FIFO_SZ ; i++)
          {
                ...
                &&(frame_offset  < p_ctx->pkt_offsets_index[i]))
            {
              if (p_ctx->pts_offsets[i] == NI_NOPTS_VALUE)
              {
                ...
                p_frame->pts = NI_NOPTS_VALUE;
                p_ctx->last_dts = p_ctx->last_pts = NI_NOPTS_VALUE;
                ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (pts NI_NOPTS_VALUE not bitstream) dts %llu pts %llu "
                       "frame_offset %" PRIu64 " i %u pkt_offsets_index_min %" PRIu64 " pkt_offsets_index %" PRIu64 "\n",
                       p_frame->dts, p_frame->pts, frame_offset, i, p_ctx->pkt_offsets_index_min[i], p_ctx->pkt_offsets_index[i]);
              }
              else
              {
                p_frame->pts = p_ctx->pts_offsets[i];
                p_ctx->last_pts = p_frame->pts;
                p_ctx->last_dts = p_frame->dts;
                ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (found pts) dts %llu pts %llu frame_offset %" PRIu64 " i %u "
            ...
            if (i == (NI_FIFO_SZ-1))
            {
              ...
              p_ctx->last_dts = p_frame->dts;              
              ni_log(NI_LOG_ERROR, "ERROR: NO pts found consider increasing NI_FIFO_SZ!\n");
              ni_log(NI_LOG_ERROR, "ni_decoder_session_read_desc: (not found use default) dts %llu pts %llu\n", p_frame->dts, p_frame->pts);
            }
          }
        }
      }
    ...
    int64_t best_effort_timestamp = guess_correct_pts(p_ctx, p_frame->pts, p_frame->dts);
    p_frame->pts = best_effort_timestamp;
    ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc: (best_effort_timestamp) pts %" PRIu64 "\n", best_effort_timestamp);
    decq_count++;
    p_ctx->frame_num++;
  }
  ...
  ni_log(NI_LOG_TRACE, "xcoder_dec_receive(): p_frame->start_of_stream=%d, p_frame->end_of_stream=%d, "
         "p_frame->video_width=%d, p_frame->video_height=%d\n",
         p_frame->start_of_stream,
         p_frame->end_of_stream,
         p_frame->video_width,
         p_frame->video_height);
  ni_log(NI_LOG_TRACE, "xcoder_dec_receive(): p_frame->data_len[0/1/2]=%d/%d/%d\n",
         p_frame->data_len[0], p_frame->data_len[1], p_frame->data_len[2]);
  ...
  if (NI_RETCODE_SUCCESS != retval)
  {
    ni_log(NI_LOG_ERROR, "ni_decoder_session_read_desc(): bad exit, retval = %d\n", retval);
    return retval;
  }
  else
  {
    ni_log(NI_LOG_TRACE, "ni_decoder_session_read_desc(): exit, rx_size = %d\n", rx_size);
    return rx_size;
  }
                               ...
                               ni_hwframe_surface_t* hwdesc)
{
  ...
  ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read(): enter\n");
  ...
  if (NI_INVALID_SESSION_ID == p_ctx->session_id)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_hwdownload_session_read(): xcoder instance id < 0, return\n");
    retval = NI_RETCODE_ERROR_INVALID_SESSION;
    LRETURN;
  }
  ...
  for (i = 0; i < NI_MAX_NUM_DATA_POINTERS - 1; i++) //discount the hwdesc
  {
    if (!p_frame->p_data[i])
    {
      ni_log(NI_LOG_ERROR, "ERROR ni_hwdownload_session_read(): No receive buffer allocated.\n");
      retval = NI_RETCODE_INVALID_PARAM;
      LRETURN;
    }
  }
  ...
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_hwdownload_session_read(): p_frame->data_len[0] = 0!.\n");
    retval = NI_RETCODE_INVALID_PARAM;
    LRETURN;
  }
...
#ifdef XCODER_IO_RW_ENABLED
  ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read totall_bytes_to_read %d max_nvme_io_size %d ylen %d cr len %d cb len %d hdr %d\n", \
         totall_bytes_to_read, \
         p_ctx->max_nvme_io_size, \
         p_frame->data_len[0], \
         p_frame->data_len[1], \
  ...
  retval = ni_config_instance_rw(p_ctx, INST_READ_CONFIG,
                                1,
  ...
  read_size_bytes = totall_bytes_to_read;
  ui32LBA = READ_INSTANCE_R(0, p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
  ...
  retval = ni_nvme_send_read_cmd(hwdesc->device_handle, NI_INVALID_DEVICE_HANDLE,
                                 p_data_buffer, read_size_bytes, ui32LBA);
  CHECK_ERR_RC(p_ctx, retval, NI_NO_CHECK_TS_NVME_CMD_OP,
               hwdesc->src_cpu, p_ctx->hw_id, &(p_ctx->session_id));
  CHECK_VPU_RECOVERY(retval);
  if (retval < 0)
  {
    ni_log(NI_LOG_ERROR, "ERROR ni_hwdownload_session_read(): nvme command failed\n");
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
    LRETURN;
  }
  else
  {
    p_ctx->decoder_read_total_num_of_bytes = p_ctx->decoder_read_total_num_of_bytes + sei_size;
    ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read success, retval %d totall_bytes_to_read include sei %d sei_size %d\n",
           retval, totall_bytes_to_read, sei_size);
  }

  ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read totall_bytes_to_read %d ylen %d cr len %d cb len %d data0 0x%x data_end 0x%x idx %d\n", \
         totall_bytes_to_read, \
         p_frame->data_len[0], \
         p_frame->data_len[1], \
         p_frame->data_len[2], \
         p_data_buffer[0], \
         p_data_buffer[totall_bytes_to_read-1], \
         totall_bytes_to_read -1);
...

  {
    ...
    if (!one_chunk_flag)
    {
      p_ctx->hwd_Frame_Idx = (uint32_t)hwdesc->ui16FrameIdx;
      p_ctx->hwd_src_cpu = (uint32_t)hwdesc->src_cpu;
      p_ctx->hwd_minor_offset = output_minor_offset;
      p_ctx->hwd_chunk_offset = output_chunk_offset;
      //p_ctx->hwd_Inst_ID = (uint32_t)hwdesc->i8InstID;
      for (; frame_chunk_counter < p_ctx->decoder_read_total_num_of_chunks - 1; frame_chunk_counter++)
      {
        p_ctx->args_decoder_read[frame_chunk_counter].opcode = nvme_cmd_xcoder_read;
        p_ctx->args_decoder_read[frame_chunk_counter].handle = hwdesc->device_handle;// p_ctx->blk_io_handle;
        p_ctx->args_decoder_read[frame_chunk_counter].p_ni_nvme_cmd = &(cmd[frame_chunk_counter]);
        p_ctx->args_decoder_read[frame_chunk_counter].data_len = (frame_remain_bytes < p_ctx->max_nvme_io_size) ? (frame_remain_bytes) : (p_ctx->max_nvme_io_size);
        p_ctx->args_decoder_read[frame_chunk_counter].p_data = p_data_buffer;
        p_ctx->args_decoder_read[frame_chunk_counter].p_result = &(nvme_result[frame_chunk_counter]);
        p_ctx->args_decoder_read[frame_chunk_counter].frame_chunk_index = ((frame_chunk_counter*p_ctx->max_nvme_io_size) / FRAME_CHUNK_INDEX_SIZE) + output_chunk_offset;
        p_ctx->args_decoder_read[frame_chunk_counter].frame_chunk_size = p_ctx->max_nvme_io_size;
        p_ctx->args_decoder_read[frame_chunk_counter].session_id = hwdesc->ui16SessionID;//  p_ctx->session_id
        //p_ctx->args_decoder_read[frame_chunk_counter].FrameIdx = (uint32_t)hwdesc->ui16FrameIdx;
        //p_ctx->args_decoder_read[frame_chunk_counter].InstID = (uint32_t)hwdesc->i8InstID;
        frame_remain_bytes = frame_remain_bytes - p_ctx->args_decoder_read[frame_chunk_counter].data_len;
        ni_log(NI_LOG_TRACE, "Decoder hwdl frame_chunk_counter %d totall_bytes_to_read %d p_ctx->decoder_read_total_num_of_chunks %d chunk index %d frame_remain_bytes %d\n", \
          frame_chunk_counter, \
          totall_bytes_to_read, \
          p_ctx->decoder_read_total_num_of_chunks, \
          p_ctx->args_decoder_read[frame_chunk_counter].frame_chunk_index, \
    ...
    assert(frame_remain_bytes <= p_ctx->max_nvme_io_size);
    last_pkt_chunk_read.opcode = nvme_cmd_xcoder_read;
    last_pkt_chunk_read.handle = hwdesc->device_handle;// p_ctx->blk_io_handle;
    last_pkt_chunk_read.p_ni_nvme_cmd = &(cmd[frame_chunk_counter]);
    last_pkt_chunk_read.data_len = frame_remain_bytes; //+ (buf_info.buf_avail_size - totall_bytes_to_read);
    last_pkt_chunk_read.p_data = p_data_buffer;
    last_pkt_chunk_read.p_result = &(nvme_result[frame_chunk_counter]);
    last_pkt_chunk_read.frame_chunk_index = ((frame_chunk_counter*p_ctx->max_nvme_io_size) / FRAME_CHUNK_INDEX_SIZE) + output_chunk_offset;
    last_pkt_chunk_read.frame_chunk_size = p_ctx->max_nvme_io_size;
    last_pkt_chunk_read.session_id = hwdesc->ui16SessionID;//  p_ctx->session_id

    WRITE_INSTANCE_SET_DW10_SUBTYPE(last_pkt_chunk_read.p_ni_nvme_cmd->cdw10, last_pkt_chunk_read.session_id);
    WRITE_INSTANCE_SET_DW11_INSTANCE(last_pkt_chunk_read.p_ni_nvme_cmd->cdw11, hwdesc->src_cpu);
    WRITE_INSTANCE_SET_DW11_PAGEOFFSET(last_pkt_chunk_read.p_ni_nvme_cmd->cdw11, last_pkt_chunk_read.frame_chunk_index);  // set page index based on 4k
    WRITE_INSTANCE_SET_DW12_ISHWDESC(last_pkt_chunk_read.p_ni_nvme_cmd->cdw12, (output_minor_offset << NI_CODEC_HW_PAYLOAD_OFFSET) | NI_CODEC_HW_ENABLE | NI_CODEC_HW_DOWNLOAD); //xcoder will upload YUV
    WRITE_INSTANCE_SET_DW12_FRAMEINSTID(last_pkt_chunk_read.p_ni_nvme_cmd->cdw12, (uint32_t)hwdesc->ui16FrameIdx);
    WRITE_INSTANCE_SET_DW15_SIZE(last_pkt_chunk_read.p_ni_nvme_cmd->cdw15, last_pkt_chunk_read.data_len);  // this one we read all the way end to the buffer
    ...
    uint32_t data_len_local = last_pkt_chunk_read.data_len;
    while (ret_last != 0) {
      ni_log(NI_LOG_TRACE, "decoder_hwdl opcode %d, handle %d, p_ni_nvme_cmd dw10:0x%8x dw11:0x%8x dw15:0x%8x,  data_len %d, pdataoff %p \n", \
        last_pkt_chunk_read.opcode, \
        last_pkt_chunk_read.handle, \
        last_pkt_chunk_read.p_ni_nvme_cmd->cdw10, \
        last_pkt_chunk_read.p_ni_nvme_cmd->cdw11, \
        last_pkt_chunk_read.p_ni_nvme_cmd->cdw15, \
        last_pkt_chunk_read.data_len, \
        ((last_pkt_chunk_read.p_data) + (last_pkt_chunk_read.frame_chunk_index - output_chunk_offset)*FRAME_CHUNK_INDEX_SIZE)
      );
      if (last_pkt_chunk_read.data_len % NI_MEM_PAGE_ALIGNMENT)
      {
        last_pkt_chunk_read.data_len = ((last_pkt_chunk_read.data_len / NI_MEM_PAGE_ALIGNMENT) * NI_MEM_PAGE_ALIGNMENT) + NI_MEM_PAGE_ALIGNMENT;
      ...
      if (ret_last != NI_RETCODE_SUCCESS)
      {
        ni_log(NI_LOG_ERROR, "decoder hwdl failed retry\n");
        last_pkt_chunk_read.p_result = 0;
        retry_count++;
        ni_usleep(100);
        if (retry_count == 6000)
        {
          ni_log(NI_LOG_ERROR, "Something is wrong as the decoder hwdl command retried 6000 times, now assert\n");
          assert(false);
        }
        continue;
      }
      else
      {
        ...
        //    (ni_metadata_dec_frame_t*)((uint8_t*)p_frame->p_buffer
        //        + p_frame->data_len[0] + p_frame->data_len[1]
        //        + p_frame->data_len[2]);//leave out data3? have xcoder send without gaps. special hw flag for this?
        //sei_size = p_meta->sei_size;
        *(last_pkt_chunk_read.p_result) = data_len_local;
        //totall_bytes_to_read = totall_bytes_to_read + sei_size;
        p_ctx->decoder_read_total_num_of_bytes = p_ctx->decoder_read_total_num_of_bytes + sei_size;
        ni_log(NI_LOG_TRACE, "decoder hwdl success, size %d local %d totall_bytes_to_read include sei %d sei_size %d\n", *(last_pkt_chunk_read.p_result), data_len_local, totall_bytes_to_read, sei_size);
        break;
      }
    }
    p_ctx->decoder_read_processed_data_len += *(last_pkt_chunk_read.p_result);
    ...
    if (!one_chunk_flag) {
      ni_log(NI_LOG_TRACE, "Totally_decoder_hwdl enqueued %d\n", frame_chunk_counter);
      // After the queue is filled up, we release the worker threads
      // Here we need to pull untill the queue is empty,
      // There is a mutex to protect each queue check
      int num_of_thread_to_release = frame_chunk_counter - 1;
      ...
      pthread_mutex_lock((p_ctx->decoder_read_mutex_len));
      while (totall_bytes_to_read != p_ctx->decoder_read_processed_data_len) {
        ni_log(NI_LOG_TRACE, "totall_bytes_to_hwdl =%d  proccessed%d\n", totall_bytes_to_read, p_ctx->decoder_read_processed_data_len);
        pthread_cond_wait((p_ctx->decoder_read_cond), (p_ctx->decoder_read_mutex_len));
      }
      pthread_mutex_unlock((p_ctx->decoder_read_mutex_len));

      ni_log(NI_LOG_TRACE, "No more decoder hwdl ongoing progressed bytes %d total len %d frame num %lu\n", p_ctx->decoder_read_processed_data_len, totall_bytes_to_read, p_ctx->frame_num);
    }
  }
#endif //XCODER_IO_RW_ENABLED
  ...
  ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read(): received data: [0x%08x]\n", rx_size);
  ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read(): p_frame->start_of_stream=%d, "
         "p_frame->end_of_stream=%d, p_frame->video_width=%d, p_frame->video_height=%d\n",
         p_frame->start_of_stream,
         p_frame->end_of_stream,
         p_frame->video_width,
         p_frame->video_height);
  ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read(): p_ctx->frame_num%ld, p_frame->data_len[0/1/2]=%d/%d/%d\n",
         p_ctx->frame_num, p_frame->data_len[0], p_frame->data_len[1], p_frame->data_len[2]);

  END;

  if (NI_RETCODE_SUCCESS != retval)
  {
    ni_log(NI_LOG_ERROR, "ni_hwdownload_session_read(): bad exit, retval = %d\n", retval);
    return retval;
  }
  else
  {
    ni_log(NI_LOG_TRACE, "ni_hwdownload_session_read(): exit, rx_size = %d\n", rx_size);
    return rx_size;
  }
...
*  \return  
*******************************************************************************/
ni_retcode_t ni_uplosader_session_close(ni_session_context_t* p_ctx)
{
  return 0;
}
...
*          negative value like NI_RETCODE_FAILURE in ni_retcode_t on failure
*******************************************************************************/
ni_retcode_t ni_hwupload_init_framepool(ni_session_context_t* p_ctx,
                                        uint32_t pool_size)
{
  ni_nvme_result_t nvme_result = 0;
  ni_nvme_command_t cmd = { 0 };
  ni_init_frames_params_t* p_init_frames_param = NULL;
  uint32_t buffer_size = sizeof(ni_encoder_config_t);
  ni_retcode_t retval = NI_RETCODE_SUCCESS;
  uint32_t ui32LBA = 0;
  int i = 0;
  ni_log(NI_LOG_TRACE, "ni_hwupload_init_framepool(): enter\n");

  if (!p_ctx)
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_hwupload_init_framepool(): NULL pointer p_config passed\n");
    retval = NI_RETCODE_INVALID_PARAM;
    LRETURN;
  }
  ...
  buffer_size = ((buffer_size + (NI_MEM_PAGE_ALIGNMENT - 1)) / NI_MEM_PAGE_ALIGNMENT) * NI_MEM_PAGE_ALIGNMENT;
  if (posix_memalign((void **)&p_init_frames_param, sysconf(_SC_PAGESIZE), buffer_size))
  {
...
#ifdef XCODER_IO_RW_ENABLED
  p_init_frames_param->width = ni_ntohs(p_ctx->active_video_width);
  p_init_frames_param->height = ni_ntohs(p_ctx->active_video_height);
  p_init_frames_param->bit_depth_factor = ni_ntohs(p_ctx->bit_depth_factor);
  p_init_frames_param->pool_size = ni_ntohs(pool_size);

  ni_log(NI_LOG_TRACE, "ni_hwupload_init_framepool():%d x %d x bitdepth %d with %d framepool \n",
         p_init_frames_param->width, p_init_frames_param->height,
         p_init_frames_param->bit_depth_factor, p_init_frames_param->pool_size);

  ui32LBA = CONFIG_INSTANCE_InitFramePool_W(p_ctx->session_id, NI_DEVICE_TYPE_DECODER);
...
#else
  QUERY_INSTANCE_SET_DW10_SUBTYPE(cmd.cdw10, p_ctx->session_id);
  cmd.cdw11 = (p_ctx->active_video_height << 16) | (p_ctx->active_video_width & 0xFFFFUL);
  cmd.cdw12 = (pool_size << 16) | (p_ctx->bit_depth_factor & 0xFFFFUL);
  ni_log(NI_LOG_TRACE, "ni_hwupload_init_framepool(): cdw11=0x%x cdw12=0x%x\n", cmd.cdw11, cmd.cdw12);

  if (ni_nvme_send_admin_cmd(nvme_admin_cmd_xcoder_init_framepool, p_ctx->device_handle, &cmd, buffer_size, p_init_frames_param, &nvme_result))
  {
    ...
    if (NI_RETCODE_SUCCESS != retval)
    {
      ni_log(NI_LOG_ERROR, "ERROR: ni_encoder_session_close failed: device_handle: %" PRIx64 ", hw_id, %u, xcoder_inst_id: %d\n", (int64_t)p_ctx->device_handle, p_ctx->hw_id, p_ctx->session_id);
    }
  ...
  END;
  aligned_free(p_init_frames_param);

  ni_log(NI_LOG_TRACE, "ni_hwupload_init_framepool(): exit\n");
...
*
*  \return - NI_RETCODE_SUCCESS on success,
*            NI_RETCODE_ERROR_INVALID_SESSION, NI_RETCODE_ERROR_NVME_CMD_FAILED
*            on failure
*******************************************************************************/
                                                    ...
                                                    uint32_t pool_size)
{
  ...
  uint32_t ui32LBA = 0;
  int i = 0;
  ni_log(NI_LOG_TRACE, "ni_config_instance_set_uploader_params(): enter\n");

  if (!p_ctx)
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_config_instance_set_uploader_params(): NULL pointer p_config passed\n");
    retval = NI_RETCODE_INVALID_PARAM;
    LRETURN;
  }
  ...
  p_cfg = (ni_encoder_config_t*)p_encoder_config;
  p_cfg->ui8bitstreamFormat = STD_AVC; //dummy value
  p_cfg->i32picWidth = p_ctx->active_video_width;
  p_cfg->i32picHeight = p_ctx->active_video_height;
  p_cfg->i32srcBitDepth = p_ctx->src_bit_depth;
  p_cfg->ui32sourceEndian = p_ctx->src_endian;
  p_cfg->ui8hwframes = NI_CODEC_HW_UPLOAD;
  p_cfg->i32maxIntraSize = pool_size; //repurposed param since intra period is more or less max frame count

  ni_log(NI_LOG_TRACE, "ni_config_instance_set_uploader_params():%d x %d x bitdepth %d with %d framepool \n",
      p_cfg->i32picWidth, p_cfg->i32picHeight, p_cfg->i32srcBitDepth, p_cfg->i32maxIntraSize);
  ...
  aligned_free(p_encoder_config);
  for (i = 0; i < 8; i++)
  {//sleep because xcoder is too slow to get ready before first write //can probably remove on ASIC
    ni_log(NI_LOG_TRACE, "sleep %d/8\n",i+1);
    ni_usleep(1000000);
  }
  ni_log(NI_LOG_TRACE, "ni_config_instance_set_encoder_params(): exit\n");
