...
void ni_close_event(ni_event_handle_t event_handle)
{
  ...
  {
    ni_log(NI_LOG_TRACE, "Warning ni_close_event: event_handle is null, return\n");
    return;
  }
...
ni_retcode_t ni_device_capability_query(ni_device_handle_t device_handle, ni_device_capability_t* p_cap)
{
  ...
  if (ni_nvme_send_admin_cmd(nvme_admin_cmd_identify, device_handle, &cmd, NI_NVME_IDENTITY_CMD_DATA_SZ,
                             p_buffer, &nvme_result))
  {
    retval = NI_RETCODE_ERROR_NVME_CMD_FAILED;
    LRETURN;
  }
...
ni_retcode_t ni_device_session_open( ni_session_context_t *p_ctx, ni_device_type_t device_type )
{
    ...
    case NI_DEVICE_TYPE_DECODER:
    {
      for (i = 0; i< NI_MAX_SESSION_OPEN_RETRIES; i++)  // 20 retries
      {
        ...
        if (NI_RETCODE_ERROR_VPU_RECOVERY == retval)
        {
          ...
          ni_usleep(NI_SESSION_OPEN_RETRY_INTERVAL_US);  // 200 us
          continue;
        }
        else
        {
  ...
  if ( 0 != pthread_create(&p_ctx->keep_alive_thread, NULL, ni_session_keep_alive_thread,
                                     ...
                                     ni_device_type_t device_type)
{
  ...
  switch (device_type)
  {
    ...
    case NI_DEVICE_TYPE_UPLOAD:
    {
        retval = ni_uploader_session_close(p_ctx, eos_recieved);
        break;
    }
#endif
    default:
  ...
  p_ctx->session_id = NI_INVALID_SESSION_ID; /*!* need set invalid after closed.
...
*                          NI_RETCODE_ERROR_INVALID_SESSION
*******************************************************************************/
int ni_device_session_init_framepool(ni_session_context_t *p_ctx, uint32_t pool_size)
{
  ni_retcode_t retval = NI_RETCODE_SUCCESS;
  ...
  retval = ni_hwupload_init_framepool(p_ctx, pool_size);
  return retval;
}
#endif
/*!******************************************************************************
...
#endif
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_device_session_write() keep alive thread has been closed, "
...
#endif
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_device_session_read() keep alive thread has been closed, "
           "hw:%d, session:%d\n", p_ctx->hw_id, p_ctx->session_id);
    return NI_RETCODE_ERROR_INVALID_SESSION;
  }
  ...
  switch (device_type)
  {
    case NI_DEVICE_TYPE_DECODER:
    {
      ...
      while (1)
      {
        ...
        {
          ni_log(NI_LOG_TRACE, "ni_device_session_read (decoder): sequence "
                         "change, frame size %ux%u %dbits -> %ux%u %dbits, continue read ...\n",
                         p_ctx->active_video_width,
                         p_ctx->active_video_height,
                         p_ctx->active_bit_depth,
                         p_data->data.frame.video_width,
                         p_data->data.frame.video_height,
                         p_data->data.frame.bit_depth);
                                   ...
                                   int hw_frame_count)
{
  ...
  if (hw_frame_count == 0)
    ...
    buffer_size = sizeof(ni_hwframe_surface_t)*hw_frame_count + metadata_size;


  // added 2 blocks of 512 bytes buffer space to handle any extra metadata
  // retrieval from fw
  buffer_size = ((buffer_size + (NI_MEM_PAGE_ALIGNMENT - 1)) / NI_MEM_PAGE_ALIGNMENT) *
                NI_MEM_PAGE_ALIGNMENT + NI_MEM_PAGE_ALIGNMENT * 3;
  ...
  if (hw_frame_count)
  {
    ...
    p_frame->data_len[3] = sizeof(ni_hwframe_surface_t) * hw_frame_count;
  }
  else
  {
...
/*!*****************************************************************************
  ...
  *                          NI_RETCODE_ERROR_MEM_ALOC
  *****************************************************************************/
ni_retcode_t ni_frame_buffer_alloc_v3(ni_frame_t* p_frame,
                                      int video_width,
                                      int video_height,
                                      int linesize[],
                                      int alignment,
                                      ...
                                      int factor)
{
  ...
  if ((!p_frame)  || (!linesize)|| (linesize[0]<=0) || (linesize[0]>NI_MAX_RESOLUTION_WIDTH*factor)
     || (video_width>NI_MAX_RESOLUTION_WIDTH) || (video_width<=0)
     || (video_height>NI_MAX_RESOLUTION_HEIGHT) || (video_height<=0))
  {
 ...
 *                        NI_RETCODE_INVALID_PARAM
 *******************************************************************************/
ni_retcode_t ni_encoder_init_default_params(ni_encoder_params_t* p_param,
                                            int fps_num,
                                            int fps_denom,
                                            long bit_rate,
                                            int width,
...
ni_retcode_t ni_decoder_init_default_params(ni_encoder_params_t* p_param,
                                            int fps_num,
                                            int fps_denom,
                                            long bit_rate,
                                            int width,
...
ni_retcode_t ni_parse_reconf_file(const char* reconf_file, int hash_map[100][10])
{
  ...
  while ((readc = fgetc(reconf)) != EOF)
  {
    ...
    {
      ni_log(NI_LOG_TRACE, "error character %c in reconfig file. this may lead to "
             "mistaken reconfiguration values \n", readc);
    }
  }
...
*
*  \param[in] p_params   Pointer to a user allocated ni_decoder_params_t
...
*                        NI_RETCODE_INVALID_PARAM
*******************************************************************************/
ni_retcode_t ni_decoder_params_set_value(ni_encoder_params_t* p_params, const char* name, char* value)
{
  bool b_error = false;
  ...
  ni_log(NI_LOG_TRACE, "ni_decoder_params_set_value(): enter\n");
  if (!p_params)
  {
    ni_log(NI_LOG_TRACE, "ERROR: ni_decoder_params_set_value(): Null pointer parameters passed\n");
    return NI_RETCODE_INVALID_PARAM;
  }
...
#define OPT2(STR1, STR2) else if (!strcasecmp(name, STR1) || !strcasecmp(name, STR2))
  ...
  OPT(NI_DEC_PARAM_OUT)
  {
    if (!strncmp(value, "hw", 2)){
      ...
      ni_log(NI_LOG_ERROR, "ERROR: ni_decoder_params_set_value(): out can only be <hw,sw> got %s\n", value);
...
#undef atof
  ...
  ni_log(NI_LOG_TRACE, "ni_decoder_params_set_value: exit, b_error=%d\n", b_error);
 ...
 *                        NI_RETCODE_INVALID_PARAM
 *******************************************************************************/
ni_retcode_t ni_encoder_params_set_value(ni_encoder_params_t* p_params,
                                         const char* name,
                                         const char* value,
...
{                                         \
  ...
  OPT( NI_ENC_PARAM_LEVEL )
  {
    ...
    if ( ( p_enc->level_idc== 0 && strcmp(value, "0") )|| 
        ( strlen(value) > 3 ) ||
        ( ni_check_level(p_enc->level_idc, 
            (ctx!=NULL)?ctx->codec_format:NI_CODEC_FORMAT_H265) != NI_RETCODE_SUCCESS) )
    {
      b_error = true;
    }
  }
  ...
  {
    COMPARE(value, NI_MAX_GOP_PRESET_IDX, NI_MIN_GOP_PRESET_IDX)
    p_enc->gop_preset_index = atoi(value);
  }
  OPT( NI_ENC_PARAM_USE_RECOMMENDED_ENC_PARAMS )
  {
    COMPARE(value, NI_MAX_USE_RECOMMENDED_ENC_PARAMS, NI_MIN_USE_RECOMMENDED_ENC_PARAMS)
    p_enc->use_recommend_enc_params = atoi(value);
  }
  OPT( NI_ENC_PARAM_ENABLE_RATE_CONTROL )
  {
    COMPARE(value, NI_MAX_BIN, NI_MIN_BIN)
  ...
  {
    COMPARE(value, NI_MAX_MIN_QP, NI_MIN_MIN_QP)
    p_enc->rc.min_qp = atoi(value);
  }
  OPT( NI_ENC_PARAM_MAX_QP )
  {
    COMPARE(value, NI_MAX_MAX_QP, NI_MIN_MAX_QP)
    p_enc->rc.max_qp = atoi(value);
  }
  OPT( NI_ENC_PARAM_MAX_DELTA_QP )
  {
    COMPARE(value, NI_MAX_MAX_DELTA_QP, NI_MIN_MAX_DELTA_QP)
  ...
  {
    COMPARE(value, NI_MAX_INTRA_PERIOD, NI_MIN_INTRA_PERIOD)
  ...
  {
    COMPARE(value, NI_MAX_INTRA_QP, NI_MIN_INTRA_QP)
  ...
  {
    COMPARE(value, NI_MAX_DECODING_REFRESH_TYPE, NI_MIN_DECODING_REFRESH_TYPE)
...
// TBD Rev. B: could be shared for HEVC and H.264
  ...
  {
    COMPARE(value, NI_MAX_CRF, NI_MIN_CRF)
  ...
  {
    COMPARE(value, NI_MAX_INTRA_REFRESH_MIN_PERIOD, NI_MIN_INTRA_REFRESH_MIN_PERIOD)
...
*          On failure
*                        NI_RETCODE_FAILURE
*                        NI_RETCODE_PARAM_ERROR_OOR and other error rc
*******************************************************************************/
ni_retcode_t ni_encoder_params_check(ni_encoder_params_t* p_params,
                                     ni_codec_format_t codec)
{
  ni_h265_encoder_params_t* p_enc = &p_params->hevc_enc_params;

  ni_log(NI_LOG_TRACE, "ni_encoder_params_check(): enter\n");
  ...
  ni_log(NI_LOG_TRACE, "ni_encoder_params_check: exit\n");
 ...
 *                        NI_RETCODE_INVALID_PARAM
 *******************************************************************************/
ni_retcode_t ni_encoder_gop_params_set_value(ni_encoder_params_t* p_params,
                                             const char* name,
                                             const char* value)
{
  bool b_error = false;
...
*  \param[in] p_serial_num  Pointer to a caller allocated ni_serial_num_t struct
*  \return On success
*                     NI_RETCODE_SUCCESS
*          On failure
*                     NI_RETCODE_INVALID_PARAM
...
*                     NI_RETCODE_ERROR_NVME_CMD_FAILED
*******************************************************************************/
ni_retcode_t ni_device_handle_map_SN(ni_device_handle_t device_handle,
                                     ni_serial_num_t *p_serial_num)
{
  ni_nvme_result_t nvme_result = 0;
  ni_nvme_command_t cmd = { 0 };
  void * p_buffer = NULL;
  ...
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_device_capability_query(): passed parameters are null!, return\n");
    retval = NI_RETCODE_INVALID_PARAM;
    LRETURN;
  }
  ...
  if (posix_memalign(&p_buffer, sysconf(_SC_PAGESIZE), NI_NVME_IDENTITY_CMD_DATA_SZ))
  {
    ni_log(NI_LOG_ERROR, "ERROR: ni_device_handle_map_SN(): Cannot allocate buffer.\n");
    retval = NI_RETCODE_ERROR_MEM_ALOC;
    LRETURN;
  }
  ...
  ni_populate_serial_number(p_serial_num, p_buffer);
...
*          device_type should be NI_DEVICE_TYPE_DECODER,
...
int ni_device_session_read_hwdesc(ni_session_context_t *p_ctx,
                                  ni_session_data_io_t *p_data)
{
  ni_log(NI_LOG_TRACE, "ni_device_session_read_hwdesc start\n");
  ni_retcode_t retval = NI_RETCODE_SUCCESS;
  if ((!p_ctx) || (!p_data))
  {
  ...
  while (1)
  {
    ...
    ni_log(NI_LOG_TRACE, "FNum %" PRIu64 ", DFVWxDFVH %u x %u, AlWid %d, AVW x AVH %u x %u\n ",
           p_ctx->frame_num,
           p_data->data.frame.video_width,
           p_data->data.frame.video_height,
           aligned_width, p_ctx->active_video_width,
    ...
    {
      ni_log(NI_LOG_ERROR, "ni_device_session_read (decoder): failure ret %d, "
             "return ..\n", retval);
      break;
    }
    else if (p_ctx->frame_num && p_data->data.frame.video_width &&
                           ...
                           ni_hwframe_surface_t* hwdesc)
{
  ...
  if ((!hwdesc) || (!p_data))
  {
    ni_log(NI_LOG_ERROR, "ni_device_session_hwdl(): Error passed parameters are null!, return\n");
    return NI_RETCODE_INVALID_PARAM;
  }
  p_ctx->session_id = hwdesc->ui16SessionID;
#ifdef XCODER_IO_RW_ENABLED
  p_ctx->blk_io_handle = hwdesc->device_handle;
#else
  p_ctx->device_handle = hwdesc->device_handle;
#endif
  ...
  ni_log(NI_LOG_TRACE, "ni_device_session_hwdl(): bit_depth_factor %u\n", p_ctx->bit_depth_factor);
                           ...
                           ni_hwframe_surface_t* hwdesc)
{
  ...
  retval = ni_hwupload_session_write(p_ctx, &(p_src_data->data.frame));
  if (retval <= 0)
  {
    return retval;
  }
                                         ...
                                         int extra_len)
{
  ...
  ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_hwenc:  extra_len=%d\n", extra_len);

  int luma_size = 0;//linesize[0] * height_aligned;
  int chroma_b_size = 0;//luma_size / 4;
  int chroma_r_size = 0;//luma_size / 4;
  int buffer_size = sizeof(ni_hwframe_surface_t) + extra_len;
  ...
  if ((p_frame->buffer_size != buffer_size) && (p_frame->buffer_size > 0))
  {
    ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_hwenc: free current p_frame,  "
           "p_frame->buffer_size=%d\n", p_frame->buffer_size);
    ni_frame_buffer_free(p_frame);
  }
  ...
  if (p_frame->buffer_size != buffer_size)
  {
    ...
    ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_hwenc: allocated new p_frame buffer\n");
  }
  else
  {
    ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_hwenc: reuse p_frame buffer\n");
  }
  ...
  p_frame->data_len[3] = sizeof(ni_hwframe_surface_t);

  p_frame->video_width = video_width;
  p_frame->video_height = height_aligned;

  ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_hwenc: success: p_frame->buffer_size="
         "%d\n", p_frame->buffer_size);
...
*          On failure    NI_RETCODE_INVALID_PARAM
*******************************************************************************/
ni_retcode_t ni_decode_buffer_free(ni_hwframe_surface_t* surface,
                                   ni_device_handle_t device_handle)
{
  int8_t i8FrameIdx;
  int8_t rsvd[2]; //reserved
  ni_retcode_t retval = NI_RETCODE_SUCCESS;
  if (surface)
  {
    if (surface->seq_change) // clear meta buffer index when sequence change happened
    {
      ni_hwframe_surface_t surface_meta = {0};
      surface_meta.ui16SessionID = surface->ui16SessionID;
      surface_meta.i8FrameIdx = NI_INVALID_HW_META_IDX;
      surface_meta.i8InstID = surface->i8InstID;
      ni_log(NI_LOG_TRACE, "ni_decode_buffer_free(): clear meta buffer\n");
      retval = ni_clear_instance_buf(&surface_meta, device_handle);
    }
    ni_log(NI_LOG_TRACE, "ni_decode_buffer_free(): Start cleaning out buffer\n");
    ni_log(NI_LOG_TRACE, "ni_decode_buffer_free(): ui16FrameIdx=%d sessionId=%d device_handle=0x%x\n",
           surface->i8FrameIdx, surface->ui16SessionID, device_handle);
    retval = ni_clear_instance_buf(surface, device_handle);
  }
  else
  {
    ni_log(NI_LOG_TRACE, "ni_decode_buffer_free(): Surface is empty\n");
  }
  return retval;
}
                                     ...
                                     uint32_t pool_size)
{
  ...
  retval = ni_hwupload_init_framepool(p_ctx, pool_size);
#else
  retval = ni_config_instance_set_uploader_params(p_ctx, pool_size);
#endif
  return retval;
}
 ...
 *
 *******************************************************************************/
ni_retcode_t ni_frame_buffer_alloc_v4(ni_frame_t* p_frame, int pixel_format,
                                      ...
                                      int linesize[], int alignment, int extra_len)
{
  ...
  height_aligned = height_aligned > NI_MIN_HEIGHT? height_aligned : NI_MIN_HEIGHT;
  ...
  if (p_frame->buffer_size != buffer_size)
  {
    ...
    p_frame->buffer_size = buffer_size;
    p_frame->p_buffer = p_buffer;
    ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_v4: allocated new p_frame buffer\n");
  }
  else
  {
    ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_v4: reuse p_frame buffer\n");
  }
  ...
  ni_log(NI_LOG_TRACE, "ni_frame_buffer_alloc_v4 success: w=%d; h=%d; aligned buffer size=%d\n",
         video_width, video_height, buffer_size);
