  ...
  for (i = 0; i < xcoder_device_cnt; i++)
  {
         ...
         (should_match_rev && ni_is_fw_compatible(device_capabilites.fw_rev))))
    {
      ...
      for (j = 0; j < total_modules; j++)
      {
        ...
        int device_cnt_so_far = (NI_DEVICE_TYPE_DECODER == device_info.device_type ?
        ...
        {
          tmp_guid = (NI_DEVICE_TYPE_DECODER == device_info.device_type ?
        ...
        if (p_device_info)
        {
          ni_log(NI_LOG_INFO, "%s h/w id %d update\n", NI_DEVICE_TYPE_DECODER == device_info.device_type ?
                 "decoder" : "encoder", device_info.hw_id);
          retval = ni_rsrc_fill_device_info(p_device_info, fmt, device_info.device_type,
...
ni_device_context_t* ni_rsrc_get_device_context(ni_device_type_t device_type, int guid)
{
  ...
  p_device_queue = (ni_device_info_t*)MapViewOfFile(map_file_handle,   // handle to map object
                                                    FILE_MAP_ALL_ACCESS, // read/write permission
                                                    0,
                                                    0,
            ...
            ni_is_fw_compatible(device_capabilites.fw_rev))))
      {
        ...
        for (j = 0; j < total_modules; j++)
        {
          ...
          if (p_device_info)
          {
            ni_log(NI_LOG_INFO, "%s h/w id %d update\n", NI_DEVICE_TYPE_DECODER == device_info.device_type ?
                   "decoder" : "encoder", device_info.hw_id);
            rc = ni_rsrc_fill_device_info(p_device_info, fmt, device_info.device_type,
                                          &device_capabilites.xcoder_devices[j]);
          }
          else
          {
...
ni_device_context_t* ni_rsrc_get_device_context(ni_device_type_t device_type, int guid)
{
  ...
  p_device_queue = (ni_device_info_t *)mmap(0, sizeof(ni_device_info_t),
...
void ni_rsrc_print_device_info(const ni_device_info_t *p_device_info)
{
  ...
  {
    ni_log(NI_LOG_INFO, "%s #%d\n", p_device_info->device_type == NI_DEVICE_TYPE_DECODER ?
...
LIB_API void ni_rsrc_print_all_devices_capability(void)
{
  ...
  {
    ni_log(NI_LOG_ERROR, "Error %d: malloc for ni_rsrc_print_all_devices_capability()",
           NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR);
  }
}
...
ni_device_info_t *ni_rsrc_get_device_info(ni_device_type_t device_type, int guid)
{
  ...
  {
    ni_log(NI_LOG_ERROR, "Error %d: malloc for ni_rsrc_get_device_info()",
           NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR);
    LRETURN;
  }
...
*******************************************************************************/
int ni_rsrc_get_available_device(int width,
                                 int height,
                                 int frame_rate,
                                 ni_codec_t codec,
...
#ifdef XCODER_IO_RW_ENABLED
    p_session_context.blk_io_handle = ni_device_open(p_device_context->p_device_info->blk_name,
                                                     &p_session_context.max_nvme_io_size);
    p_session_context.device_handle = p_session_context.blk_io_handle;
#else
    p_session_context.device_handle = ni_device_open(p_device_context->p_device_info->dev_name,
                                                     &p_session_context.max_nvme_io_size);
#endif
...
static void ni_rsrc_move_device_to_end_of_pool(ni_device_type_t type,
                                               int guid,
                                               ni_device_pool_t *p_device_pool)
{
  ...
  ni_log(NI_LOG_INFO, "Moving %s %d to queue end ..\n", type == NI_DEVICE_TYPE_DECODER ?
         "decoder" : "encoder", guid);

  coders = (type == NI_DEVICE_TYPE_DECODER ? p_device_pool->p_device_queue->decoders :
                                             p_device_pool->p_device_queue->encoders);

  count = (type == NI_DEVICE_TYPE_DECODER ? p_device_pool->p_device_queue->decoders_cnt :
                                           ...
                                           unsigned long* p_load)
{
  ...
  if (p_device_pool)
  {
    ...
    coders = ( NI_DEVICE_TYPE_DECODER == device_type ? p_device_pool->p_device_queue->decoders :
                                                       p_device_pool->p_device_queue->encoders);

    count = ( NI_DEVICE_TYPE_DECODER == device_type ? p_device_pool->p_device_queue->decoders_cnt :
                                                      p_device_pool->p_device_queue->encoders_cnt);
...
*          calling ni_rsrc_free_device_context.
*******************************************************************************/
ni_device_context_t *ni_rsrc_allocate_direct(ni_device_type_t device_type,
                                             int guid,
                                             ni_codec_t codec,
                                             int width,
                                             int height,
...
*
*******************************************************************************/
int ni_rsrc_check_sw_instance(ni_device_context_t *p_device_context,
                              ni_device_type_t device_type)
{
  int retval = NI_RETCODE_SUCCESS;
