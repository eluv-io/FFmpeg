...
uint32_t ni_get_kernel_max_io_size(const char * p_dev)
{
  ...
  ni_log(NI_LOG_DEBUG, "\nMAX NVMe IO Size of %d was calculated for this platform "
         "and will be used unless overwritten by user settings\n", io_size);
  fflush(stderr);
...
void ni_dec_fme_buffer_pool_free(ni_buf_pool_t *p_buffer_pool)
{
  ...
  {
    ni_log(NI_LOG_INFO, "ni_dec_fme_buffer_pool_free: NOT allocated\n");
  }
}
...
ni_retcode_t ni_find_blk_name(const char *p_dev, char *p_out_buf, int out_buf_len)
{
  ...
  // p_dev[5] is p_dev without '/dev/'
  snprintf(cmd, sizeof(cmd) - 1, "ls /sys/class/nvme/%s/ | grep %s", &p_dev[5], NI_NVME_PREFIX);
#else
  // Note, we are using udevadm through terminal instead of libudev.c to avoid requiring extra kernel dev packges
  snprintf(cmd, sizeof(cmd) - 1, "ls /sys/`udevadm info -q path -n %s` | grep -m 1 -P \"%s(\\d+c)?\\d+n\\d+\"",
           p_dev, NI_NVME_PREFIX);
#endif
...
#else
    // On systems with virtualized NVMe functions sysfs will include context index in namespaceID.
    // But, udev will not.
    // Thus, it will be necessary to remove the context index from the namespaceID
    // when mapping deviceID to namespaceID through sysfs.
                              ...
                              ni_queue_buffer_pool_t *p_buffer_pool)
{
  ...
  ni_log(NI_LOG_TRACE, "ni_timestamp_get: getting timestamp with frame_info=%" PRIu64 "\n", frame_info);
  ...
  ni_log(NI_LOG_TRACE, "ni_timestamp_get: timestamp=%" PRId64 ", frame_info=%" PRIu64 ", err=%d\n",
                                 ...
                                 ni_queue_buffer_pool_t *p_buffer_pool)
{
  ...
  ni_log(NI_LOG_TRACE, "ni_timestamp_get: getting timestamp with frame_offset=%" PRIu64 "\n", frame_offset);
  ...
  ni_log(NI_LOG_TRACE, "ni_timestamp_get: timestamp=%" PRId64 ", frame_offset=%" PRIu64 ", err=%d\n",
                           ...
                           int64_t timestamp)
{
  ...
  {
    ni_log(NI_LOG_DEBUG, "ni_queue_push: queue overflow, remove oldest entry, count=%d\n", p_queue->count);
    // ni_assert(0);
    //Remove oldest one
...
ni_retcode_t ni_queue_print(ni_queue_t *p_queue)
{
  ...
  while (temp)
  {
    ...
    if (ltime)
    {
      ...
      ni_log(NI_LOG_TRACE, " %s [%" PRIu64 ", %" PRIu64 "]", buff, temp->timestamp, temp->frame_info);
    }
    temp = temp->p_next;
  }
  ...
  while (temp)
  {
    ni_log(NI_LOG_TRACE, " [%" PRIu64 ", %" PRIu64 "]\n", temp->timestamp, temp->frame_info);
    temp = temp->p_prev;
  }
  ni_log(NI_LOG_TRACE, "\n");
...
{
  // width (in pixels) is multiples of 32
  plane_stride[0] = ((width + 31) / 32) * 32;
  if (plane_stride[0] < NI_MIN_WIDTH)
  {
    plane_stride[0] = NI_MIN_WIDTH;
  }
  plane_stride[0] *= bit_depth_factor;
  plane_stride[1] = plane_stride[2] = plane_stride[0] / 2;

  // height (in lines) is multiples of 8 for H.265, 16 for H.264
  plane_height[0] = ((height + 7) / 8) * 8;
  if (is_h264)
  {
    plane_height[0] = ((height + 15) / 16) * 16;
  }
  if (plane_height[0] < NI_MIN_HEIGHT)
  {
                        ...
                        int src_height[NI_MAX_NUM_DATA_POINTERS])
{
  ...
  {
    ni_log(NI_LOG_TRACE, "ni_copy_hw_yuv420p: src and dst identical, return\n");
    return;
  }

  ni_log(NI_LOG_TRACE, "ni_copy_hw_yuv420p dst_stride %d/%d/%d src_stride "
         "%d/%d/%d dst_height %d/%d/%d src_height %d/%d/%d\n",
  ...
  {
    int height = (src_height[i] < dst_height[i] ?
    ...
    if (0 == i) // Y
    {
      pad_len = dst_stride[i] / bit_depth_factor - frame_width;
    }
    else if (1 == i)
    {
    ...
    pad_len_bytes = (i == 0 ? pad_len * bit_depth_factor :
                     pad_len * bit_depth_factor / 2);
    ni_log(NI_LOG_TRACE, "ni_copy_hw_yuv420p plane %d stride padding: %d pixel "
           "(%d bytes), copy height: %d.\n", i, pad_len, pad_len_bytes, height);

    for (; height > 0; height--)
    {
      memcpy(dst, src, (src_stride[i] < dst_stride[i] ?
    ...
    if (padding_height > 0)
    {
      ni_log(NI_LOG_TRACE, "ni_copy_hw_yuv420p plane %d padding height: %d\n",
             i, padding_height);
...
ni_lat_meas_q_t * ni_lat_meas_q_create(unsigned capacity)
{
    ...
    if(!queue)
    {
      ni_log(NI_LOG_ERROR, "ERROR %d: Failed to allocate memory for lat_meas-queue queue",
             NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR);
      return NULL;
    }
    ...
    if(!queue->array)
    {
      ni_log(NI_LOG_ERROR, "ERROR %d: Failed to allocate memory for lat_meas_queue queue->array",
             NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR);
      return NULL;
    }
    return queue;
