 ...
 *  \return
 *******************************************************************************/
int32_t ni_nvme_send_admin_pass_through_command(ni_device_handle_t handle,
...
static ni_retcode_t ni_nvme_get_identity(HANDLE handle,
                                         HANDLE event_handle,
                                         PVOID p_identity)
{
  ...
  {
    PNVME_IDENTIFY_CONTROLLER_DATA p_std_identity = (PNVME_IDENTIFY_CONTROLLER_DATA)(((PUCHAR)p_buffer) +
                                                    buffer_len - NVME_MAX_LOG_SIZE);
...
int ni_nvme_enumerate_devices(char ni_devices[][MAX_DEVICE_NAME_LEN],
                              int  max_handles)
{
    ...
    else
    {
      ...
      if (NI_RETCODE_SUCCESS == rc)
      {
            ...
            (p_ni_id_data->device_is_xcoder))
        {
          ...
          memcpy(firmware_revision, p_ni_id_data->ai8Fr, sizeof(p_ni_id_data->ai8Fr));
          firmware_revision[sizeof(p_ni_id_data->ai8Fr)] = 0;

          memset(model_name, 0, sizeof(model_name));
          memcpy(model_name, p_ni_id_data->ai8Mn, sizeof(p_ni_id_data->ai8Mn));
          model_name[sizeof(p_ni_id_data->ai8Mn)] = 0;

          memset(serial_num, 0, sizeof(serial_num));
          memcpy(serial_num, p_ni_id_data->ai8Sn, sizeof(p_ni_id_data->ai8Sn));
          serial_num[sizeof(p_ni_id_data->ai8Sn)] = 0;
      ...
      if (NI_RETCODE_SUCCESS == rc)
      {
        ...
        memset(firmware_revision, 0, sizeof(firmware_revision));
        memcpy(firmware_revision, p_std_identity->FR, sizeof(p_std_identity->FR));
        firmware_revision[sizeof(p_std_identity->FR)] = 0;
...
inline int32_t ni_nvme_send_win_io_command(ni_nvme_admin_opcode_t nvme_opcode,
                                           ...
                                           BOOL b_admin_command)
{
  ...
  switch (nvme_opcode)
  {
    ...
    default:
    {
      ni_log(NI_LOG_TRACE, "ERROR: ni_nvme_send_win_io_command() unsupported NVMe opcode: %d, "
             "isAdminOpcode: %d FATAL.\n", nvme_opcode, b_admin_command);
      LRETURN;
      break;
    }
  }
  ...
  if (FALSE == b_retval)
  {
    ...
    NVME_ERROR_INFO_LOG* ptrLog = (NVME_ERROR_INFO_LOG*)(p_buffer + FIELD_OFFSET(STORAGE_PROTOCOL_COMMAND, Command) +
  ...
  for (scsi_port = 0; scsi_port < max_handles; scsi_port++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        /* make the fw revision string null terminated */
        memset(firmware_revision, 0, sizeof(firmware_revision));
        memcpy(firmware_revision, p_identity_data->FR, sizeof(p_identity_data->FR));
        firmware_revision[sizeof(p_identity_data->FR)] = 0;
                                           ...
                                           BOOL b_admin_command)
{
  ...
  {
    DWORD error = NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR;
    ni_log(NI_LOG_TRACE, "ERROR: ni_nvme_send_win_io_command() Sending Opcode %02X FAILED!, "
           "Last Return Code: %d\n", nvme_opcode, error);
  }
  else
  {
...
void ni_parse_lba(uint64_t lba)
{
  ...
  uint16_t session_id = (uint16_t)(lba_high >> (NI_SESSION_ID_OFFSET - NI_INSTANCE_TYPE_OFFSET));
                              ...
                              uint32_t lba)
{
  ...
  if (rc == FALSE)
  {
    ...
    ni_log(NI_LOG_TRACE, "ni_nvme_send_read_cmd() ReadFile handle=%" PRIx64 ", event_handle=%" PRIx64 ", "
  ...
  {
      ni_log(NI_LOG_TRACE, "ni_nvme_send_read_cmd() ReadFile success handle=%" PRIx64 ", "
                               ...
                               uint32_t lba)
{
  ...
  {
    rc = NI_XCODER_NUM_OF_SYSTEM_LAST_ERROR;
    ni_log(NI_LOG_TRACE, "ni_nvme_send_write_cmd() WriteFile handle=%" PRIx64 ", event_handle=%" PRIx64 ", "
           "lba=0x%x, len=%d, rc=%d\n", (int64_t)handle, (int64_t)event_handle, (lba<<3), data_len, rc);
    if(rc == ERROR_IO_PENDING)
    {
      rc = WaitForSingleObject(event_handle, 100);
  ...
  {
      ni_log(NI_LOG_TRACE, "ni_nvme_send_write_cmd() ReadFile success handle=%" PRIx64 ", "
             "event_handle=%" PRIx64 ", lba=0x%x, len=%d, rc=%d\n", 
             (int64_t)handle, (int64_t)event_handle, (lba<<3), data_len, rc);
      rc = NI_RETCODE_SUCCESS;
  }
