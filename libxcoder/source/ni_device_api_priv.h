 ...
 *   @date          April 1, 2018
 *
 *   \brief         Private definitions used by main ni_device_api file
 *
 *   @author        
 *
 ******************************************************************************/
...
typedef struct __attribute__((packed)) _ni_inst_config_rw
{
  uint8_t ui8Enable;
  uint8_t ui8HWAccess;
  union
...
typedef struct _ni_get_session_id
{
  uint32_t session_id;          /*! Session ID */
} ni_get_session_id_t;

typedef struct _ni_encoder_session_open_info
{
  uint32_t codec_format;        /*! Device Type, Either NI_CODEC_FORMAT_H264 or NI_CODEC_FORMAT_H265 */
...
} ni_encoder_session_open_info_t;

typedef struct _ni_decoder_session_open_info
{
  uint32_t codec_format;        /*! Device Type, Either NI_CODEC_FORMAT_H264 or NI_CODEC_FORMAT_H265 */
  uint32_t model_load;
  ...
  uint32_t hw_desc_mode;        // hw action mode
  uint32_t reserved[2];
} ni_decoder_session_open_info_t;

typedef struct _ni_session_closed_status
{
  uint32_t session_closed;
} ni_session_closed_status_t;

typedef struct _ni_instance_dec_packet_info
{
  uint32_t packet_size;   // encoder packet actual size
} ni_instance_dec_packet_info_t;
...
typedef struct _ni_encoder_config_t
{
  uint8_t ui8bitstreamFormat; /*!*< The standard type of bitstream in encoder operation.
                                It is one of STD_AVC and STD_HEVC, use enums from ni_bitstream_format_t. */
  int32_t i32picWidth;        /*!*< The width of a picture to be encoded in unit of sample. */
  int32_t i32picHeight;       /*!*< The height of a picture to be encoded in unit of sample. */
  ...
  uint8_t  ui8Reserved[6]; /**< reserved bytes **/
 ...
 *
 *  \param ni_frame_t* p_frame - ni_frame_t
 ...
 *  \return
 *******************************************************************************/
void ni_params_print(ni_encoder_params_t * const p_encoder_params, ni_device_type_t device_type);
 ...
 *  \return
 *******************************************************************************/
int ni_decoder_session_query(ni_session_context_t *p_ctx);
...
//int ni_encoder_session_reconfig(ni_session_context_t *p_ctx, ni_session_config_t *p_config,
...
*          negative value like NI_RETCODE_FAILURE in ni_retcode_t on failure
*******************************************************************************/
int ni_hwupload_init_framepool(ni_session_context_t* p_ctx,
                               uint32_t pool_size);
...
*
*  \param   ni_frame_surface1_t* surface - target hardware descriptor
