...
#if (LIBAVCODEC_VERSION_MAJOR >= 59 || (LIBAVCODEC_VERSION_MAJOR >= 58 && LIBAVCODEC_VERSION_MINOR > 54))
...
static int ni_device_init(InputStream *ist)
{
    ...
    err = av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_NI_LOGAN,
...
int ni_logan_init(AVCodecContext *s)
{
  ...
  AVNILOGANFramesContext *frames_hwctx;
  int ret;
  ...
  frames_ctx->format = AV_PIX_FMT_NI_LOGAN;
  frames_ctx->sw_format = s->sw_pix_fmt;
  frames_ctx->initial_pool_size = 64;
  frames_hwctx->frame_type = NI_LOGAN_MEMTYPE_VIDEO_MEMORY_DECODER_TARGET;
